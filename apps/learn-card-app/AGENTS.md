...
| Path                   | Purpose                                                                     |
| ---------------------- | --------------------------------------------------------------------------- |
| `environments/`        | Per-tenant config, assets, and stage overlays. See `environments/README.md` |
| `src/config/`          | Runtime bootstrap (`bootstrapTenantConfig.ts`, `brandingAssets.ts`)         |
| `src/theme/`           | JSON theme engine, validators, theme store                                  |
| `scripts/`             | Developer tooling (`lc.ts`, `create-tenant.ts`, `config-editor.ts`, etc.)   |
| `tools/config-editor/` | Static HTML config/theme editor UI                                          |
...
Several files are generated by `scripts/prepare-native-config.ts` and must not be committed:
...
| `ios/App/App.xcodeproj/project.pbxproj.template`       | `ios/App/App.xcodeproj/project.pbxproj`       |
| `android/app/build.gradle.template`                    | `android/app/build.gradle`                    |
| `android/app/src/main/AndroidManifest.xml.template`    | `android/app/src/main/AndroidManifest.xml`    |
| `android/app/src/main/res/values/strings.xml.template` | `android/app/src/main/res/values/strings.xml` |

Firebase configs, Capacitor configs, icons, splash screens, manifests, `Podfile.lock`, and `public/tenant-config.json` are also gitignored and generated from tenant assets. On a fresh clone, run `bun run lc dev` (which runs `prepare-native-config.ts`) to generate them.

### Developer CLI

```bash
bun run lc          # Interactive menu — pick tenant, stage, launch mode
bun run lc dev vetpass # Full stack for a specific tenant
...
bunx playwright test wallet-credentials.spec.ts app-store.spec.ts

# Run headed (visible browser)
bunx playwright test wallet-credentials.spec.ts --headed

# No retries (faster feedback during development)
bunx playwright test wallet-credentials.spec.ts --retries=0
```
...
The **mocked tier** replays a recorded HAR for auth/boot + reads and serves tRPC mutations
from typed, drift-guarded stubs — no docker. Prefer it for fast, frequently-run coverage of
UI logic and flows within the recorded surface; keep genuinely backend-dependent flows
(real signing, network delivery, claim links) in the real tier as the semantic net.
