...
Firebase configs, Capacitor configs, icons, splash screens, manifests, and `public/tenant-config.json` are also gitignored and generated from tenant assets. On a fresh clone, run `pnpm lc dev` (which runs `prepare-native-config.ts`) to generate them.
