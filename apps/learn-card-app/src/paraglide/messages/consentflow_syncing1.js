...
const de_consentflow_syncing1 = /** @type {(inputs: Consentflow_Syncing1Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Synchronisiere...`)
};
...
const ko_consentflow_syncing1 = /** @type {(inputs: Consentflow_Syncing1Inputs) => LocalizedString} */ () => {
...
*/
const consentflow_syncing1 = /** @type {((inputs?: Consentflow_Syncing1Inputs, options?: { locale?: "en" | "es" | "de" | "ar" | "fr" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Consentflow_Syncing1Inputs, { locale?: "en" | "es" | "de" | "ar" | "fr" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	const locale = experimentalStaticLocale ?? options.locale ?? getLocale()
	if (locale === "en") return en_consentflow_syncing1(inputs)
	if (locale === "es") return es_consentflow_syncing1(inputs)
	if (locale === "de") return de_consentflow_syncing1(inputs)
	if (locale === "ar") return ar_consentflow_syncing1(inputs)
	if (locale === "fr") return fr_consentflow_syncing1(inputs)
	return ko_consentflow_syncing1(inputs)
});
