...
const de_recovery_verifyidentity1 = /** @type {(inputs: Recovery_Verifyidentity1Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Überprüfe deine Identität`)
};
...
const ko_recovery_verifyidentity1 = /** @type {(inputs: Recovery_Verifyidentity1Inputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`신원 확인`)
};
...
*/
const recovery_verifyidentity1 = /** @type {((inputs?: Recovery_Verifyidentity1Inputs, options?: { locale?: "en" | "es" | "de" | "ar" | "fr" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Recovery_Verifyidentity1Inputs, { locale?: "en" | "es" | "de" | "ar" | "fr" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	const locale = experimentalStaticLocale ?? options.locale ?? getLocale()
	if (locale === "en") return en_recovery_verifyidentity1(inputs)
	if (locale === "es") return es_recovery_verifyidentity1(inputs)
	if (locale === "de") return de_recovery_verifyidentity1(inputs)
	if (locale === "ar") return ar_recovery_verifyidentity1(inputs)
	if (locale === "fr") return fr_recovery_verifyidentity1(inputs)
	return ko_recovery_verifyidentity1(inputs)
});
