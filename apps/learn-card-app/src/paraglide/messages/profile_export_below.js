...
const de_profile_export_below = /** @type {(inputs: Profile_Export_BelowInputs) => LocalizedString} */ () => {
...
const ko_profile_export_below = /** @type {(inputs: Profile_Export_BelowInputs) => LocalizedString} */ () => {
...
*/
const profile_export_below = /** @type {((inputs?: Profile_Export_BelowInputs, options?: { locale?: "en" | "es" | "de" | "ar" | "fr" | "ko" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Profile_Export_BelowInputs, { locale?: "en" | "es" | "de" | "ar" | "fr" | "ko" }, {}>} */ ((inputs = {}, options = {}) => {
	const locale = experimentalStaticLocale ?? options.locale ?? getLocale()
	if (locale === "en") return en_profile_export_below(inputs)
	if (locale === "es") return es_profile_export_below(inputs)
	if (locale === "de") return de_profile_export_below(inputs)
	if (locale === "ar") return ar_profile_export_below(inputs)
	if (locale === "fr") return fr_profile_export_below(inputs)
	return ko_profile_export_below(inputs)
});
