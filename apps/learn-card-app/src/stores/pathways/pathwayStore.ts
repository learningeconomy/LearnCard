...
const rollupInDraft = (draft: PathwayStoreState, completedBefore: ReadonlySet<string>): void => {
    ...
    for (const [id, pathway] of Object.entries(draft.pathways)) {
        const isNowCompleted = computePathwayProgress(pathway).destinationCompleted;
...
export const pathwayStore = createStore('pathwayStore')<PathwayStoreState>(
    ...
    .extendActions(set => ({
        ...
        upsertPathway: (pathway: Pathway) => {
            ...
            const completedBefore = snapshotCompletedPathways(pathwayStore.get.pathways());
        ...
        removePathway: (pathwayId: string) => {
            set.state(draft => {
                ...
                if (draft.activePathwayId !== null && toDelete.has(draft.activePathwayId)) {
