...
export const FamilyCMSWallpaperSelect: React.FC<{
    state: FamilyCMSState;
    setState: React.Dispatch<React.SetStateAction<FamilyCMSState>>;
}> = ({ state, setState }) => {
    const [wallpaper, setWallpaper] = useState<string>(state?.appearance?.backgroundImage ?? '');
    ...
    return (
        <div className="flex items-center justify-between w-full bg-grayscale-100 p-2 pb-4 pl-[10px] mb-2 border-b-white border-b-solid border-b-[2px]">
...
export default FamilyCMSWallpaperSelect;
