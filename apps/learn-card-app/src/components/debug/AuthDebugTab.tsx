...
// ---------------------------------------------------------------------------
// Exported status helpers for use by DevDebugPanel FAB
// ---------------------------------------------------------------------------

export { getMeta, STATUS_META };
...
export const AuthDebugTab: React.FC = () => {
    ...
    const handleInvalidateSession = useCallback(async () => {
        ...
        try {
            ...
            alert(
                'Auth session invalidated. Auth session is now expired.\nTry opening Account Recovery to see the re-auth gate.'
            );
        } catch (e) {
    ...
    const handleExportEvents = useCallback(async () => {
        ...
        copyToClipboard('export', JSON.stringify(exportData, null, 2));
    }, [
        state,
        did,
        didKeyDid,
    ...
    return (
        <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 mb-1.5">
                    <div
                        className={`w-2.5 h-2.5 rounded-full ${meta.dot} ${
                            ['authenticating', 'checking_key_status', 'deriving_key'].includes(
                                state.status
                            )
                                ? 'animate-pulse'
                                : ''
            ...
            {/* ── Environment Config ── */}
            <Section title="Environment" icon={<Settings className="w-3 h-3 text-gray-500" />}>
                <KVRow
                    label="Auth Provider"
                ...
                <KVRow
                    ...
                    value={
                        (authConfig.providerConfig.web3Auth?.clientId as string)
                            ? truncate(authConfig.providerConfig.web3Auth.clientId as string, 20)
                            : '—'
                    }
                    copied={copied}
                    onCopy={copyToClipboard}
                ...
                <KVRow
                    label="Network"
                    value={(authConfig.providerConfig.web3Auth?.network as string) || '—'}
                    copied={copied}
                    onCopy={copyToClipboard}
                />
                <KVRow
                    label="Verifier ID"
                    value={(authConfig.providerConfig.web3Auth?.verifierId as string) || '—'}
                    copied={copied}
                    onCopy={copyToClipboard}
                />
                <KVRow
                    ...
                    value={
                        (authConfig.providerConfig.web3Auth?.rpcTarget as string)
                            ? truncate(authConfig.providerConfig.web3Auth.rpcTarget as string, 30)
                            : '—'
                    }
                    copied={copied}
                    onCopy={copyToClipboard}
            ...
            >
                ...
                <KVRow
                    label="Profile ID"
                    value={lcnProfile?.profileId ? truncate(lcnProfile.profileId, 20) : '—'}
                    copied={copied}
                    onCopy={copyToClipboard}
                />
                <KVRow
                            ...
                            serverState.primaryDid !== didKeyDid && (
                                <div className="flex items-center gap-1 mt-1 text-[9px]">
                                    ...
                                    <span className="text-red-400">
                                        Server primaryDid ≠ did:key — will trigger false recovery!
                                    </span>
                                </div>
                            )}
                    </>
                ...
                {!serverState ? (
                    ...
                    <>
                        ...
                        <KVRow
                            ...
                            value={
                                serverState.primaryDid ? truncate(serverState.primaryDid, 28) : '—'
                            }
                            copied={copied}
                            onCopy={copyToClipboard}
                        />
                                    ...
                                    {rm.shareVersion != null && (
                                        <span
                                            className={`text-[8px] font-mono px-1 py-0.5 rounded ${
                                                ...
                                                rm.shareVersion !== serverState.shareVersion
                ...
                badge={
                    <div className="flex items-center gap-1">
                        ...
                        >
                            {allShares.length === 0
                                ? 'none'
                                : `${allShares.length} share${allShares.length > 1 ? 's' : ''}`}
                        </span>
            ...
            <Section title="Danger Zone" icon={<Zap className="w-3 h-3 text-red-400" />}>
                <div className="space-y-2 py-1">
                    <div>
                        ...
                        <p className="text-[8px] text-gray-600 mt-0.5">
                            Signs out of Firebase without clearing coordinator state. Simulates
                            session expiration.
                        </p>
                    </div>
                </div>
                    ...
                    events.length > 0 ? (
                        <div className="flex items-center gap-0.5">
                            <button
                                ...
                                className="p-1 rounded hover:bg-gray-600 transition-colors"
                                title={
                                    copied === 'export' ? 'Copied!' : 'Export full debug snapshot'
                                }
                            >
                                {copied === 'export' ? (
