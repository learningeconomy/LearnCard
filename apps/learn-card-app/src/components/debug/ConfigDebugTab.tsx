...
export const ConfigDebugTab: React.FC = () => {
    ...
    const inheritedDefaults = useMemo(() => {
        ...
        return TENANT_UNIQUE_FIELDS.filter(({ path }) => {
            ...
            const defaultVal = getNestedValue(
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="rounded-lg p-3 bg-indigo-950/40 ring-1 ring-indigo-700">
                ...
                <KVRow
                    label="Default Theme"
                    value={config.branding.defaultTheme}
                    copied={copied}
                    onCopy={setCopied}
                />
                <KVRow
                    label="Allowed Themes"
                    value={config.branding.allowedThemes?.join(', ') ?? '(all)'}
                    mono={false}
                    copied={copied}
                    onCopy={setCopied}
                />
                ...
                <div className="rounded-lg p-2.5 bg-yellow-950/30 ring-1 ring-yellow-700/50">
                    <div className="flex items-center gap-1.5 mb-1.5">
                        ...
                        <span className="text-[10px] font-semibold text-yellow-400">
                            {inheritedDefaults.length} field
                            {inheritedDefaults.length > 1 ? 's' : ''} inherited from LearnCard
                            defaults
                        </span>
                    </div>
                    ...
                    <div className="space-y-0">
                        {inheritedDefaults.map(({ path, label }) => (
                            <div
                                key={path}
                                className="flex items-center justify-between text-[10px] py-[2px] border-t border-yellow-800/30"
                            >
                                <span className="text-yellow-500/80">{label}</span>
                                <span className="text-yellow-400/60 font-mono text-[9px]">
                                    {path}
                                </span>
                            </div>
                        ))}
            ...
            {/* ── API Endpoints ── */}
            <Section title="API Endpoints" icon={<Database className="w-3 h-3 text-gray-500" />}>
                <KVRow
                    label="Brain Service"
                    value={config.apis.brainService ? truncate(config.apis.brainService, 40) : '—'}
                    copied={copied}
                    onCopy={setCopied}
                />
                <KVRow
                    label="Brain API"
                    value={
                        config.apis.brainServiceApi
                            ? truncate(config.apis.brainServiceApi, 40)
                            : '—'
                    }
                    copied={copied}
                    onCopy={setCopied}
                ...
                <KVRow
                    label="Cloud Service"
                    value={config.apis.cloudService ? truncate(config.apis.cloudService, 40) : '—'}
                    copied={copied}
                    onCopy={setCopied}
                />
                <KVRow
                    label="LCA API"
                    value={config.apis.lcaApi ? truncate(config.apis.lcaApi, 40) : '—'}
                    copied={copied}
                    onCopy={setCopied}
                />
                <KVRow
                    label="xAPI"
                    value={config.apis.xapi ? truncate(config.apis.xapi, 40) : '—'}
                    copied={copied}
                    onCopy={setCopied}
                />
                <KVRow
                    label="AI Service"
                    value={config.apis.aiService ? truncate(config.apis.aiService, 40) : '—'}
                    copied={copied}
                    onCopy={setCopied}
                />
            </Section>
            ...
            {/* ── Links ── */}
            <Section title="External Links" icon={<Link2 className="w-3 h-3 text-gray-500" />}>
                <KVRow
                    label="App Store"
                    value={config.links.appStoreUrl ? truncate(config.links.appStoreUrl, 40) : '—'}
                    copied={copied}
                    onCopy={setCopied}
                />
                <KVRow
                    label="Play Store"
                    value={
                        config.links.playStoreUrl ? truncate(config.links.playStoreUrl, 40) : '—'
                    }
                    copied={copied}
                    onCopy={setCopied}
                />
            ...
            >
                {featureFlags.map(({ key, value }) => (
                    <div
                        key={key}
                        className="flex items-center justify-between text-[11px] py-[3px] border-t border-gray-700/40"
                    >
            ...
            {/* ── Observability ── */}
            <Section title="Observability" icon={<Eye className="w-3 h-3 text-gray-500" />}>
                <KVRow
                    label="Sentry DSN"
                    value={
                        config.observability.sentryDsn
                            ? truncate(config.observability.sentryDsn, 36)
                            : '—'
                    }
                    copied={copied}
                    onCopy={setCopied}
                ...
                <KVRow
                    label="LaunchDarkly"
                    value={
                        config.observability.launchDarklyClientId
                            ? truncate(config.observability.launchDarklyClientId, 20)
                            : '—'
                    }
                    copied={copied}
                    onCopy={setCopied}
                ...
                <KVRow
                    label="Userflow"
                    value={
                        config.observability.userflowToken
                            ? truncate(config.observability.userflowToken, 20)
                            : '—'
                    }
                    copied={copied}
                    onCopy={setCopied}
            ...
            {config.native && (
                ...
                >
                    <KVRow
                        label="Bundle ID"
                        value={config.native.bundleId ?? '—'}
                        copied={copied}
                        onCopy={setCopied}
                    />
                    <KVRow
                        label="Display Name"
                        value={config.native.displayName ?? '—'}
                        mono={false}
                        copied={copied}
                        onCopy={setCopied}
                    />
                    <KVRow
                        label="Deep Link Domains"
                        value={config.native.deepLinkDomains?.join(', ') ?? '—'}
                        mono={false}
                        copied={copied}
                        onCopy={setCopied}
                    />
