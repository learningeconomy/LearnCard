...
import { WALLET_ROUTE_PRELOAD } from '../../Routes';
 ...
 * preload in WALLET_ROUTE_PRELOAD. Keeps the current page mounted during the
 * await so the Suspense fallback never fires for chunk-load delays.
 *
...
const PreloadingLink: React.FC<LinkProps> = ({ to, onClick, children, ...rest }) => {
    ...
    const preload = path ? WALLET_ROUTE_PRELOAD[path] : undefined;
