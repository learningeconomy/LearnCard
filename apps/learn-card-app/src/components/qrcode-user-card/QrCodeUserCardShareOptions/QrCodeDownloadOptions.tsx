    ...
    return (
        <div className="w-full flex items-center justify-center my-6 px-6">
            <div className="w-full max-w-[400px]">
                <h2 className="text-[24px] font-semibold text-grayscale-900">{m['scanner.downloadQrCode']()}</h2>
                ...
                {userQrCodeDownloadOptions.map(option => {
                    let text = option.type === QrCodeDownloadOptionsEnum.saveToPhotos
                        ? (Capacitor.isNativePlatform() ? m['scanner.saveToPhotos']() : m['scanner.saveAsPhoto']())
                        : (Capacitor.isNativePlatform() ? m['scanner.saveToFiles']() : m['scanner.saveAsFile']());
