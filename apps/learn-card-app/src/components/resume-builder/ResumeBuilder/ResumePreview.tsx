...
import { resumeBuilderStore, RESUME_SECTIONS, ResumeSectionKey } from '../../../stores/resumeBuilderStore';
...
const ResumePreview: React.FC = () => {
    ...
    const hasAnyContent = useMemo(() => {
        ...
        const hasCredentials = Object.values(selectedCredentialUris).some(arr => arr && arr.length > 0);
    ...
    if (!hasAnyContent) {
        return (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-grayscale-400 select-none">
                ...
                <p className="text-xs text-center max-w-[200px]">Fill in your details and select credentials from the panel</p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full max-w-[760px] mx-auto bg-white shadow-[0_4px_24px_rgba(0,0,0,0.10)] rounded-lg p-10 min-h-[1000px] font-sans">
            {/* Header */}
            {(personalDetails.name || personalDetails.email || personalDetails.phone || personalDetails.location) && (
                ...
                return (
                    <div key={section.key} className="mb-6">
                        ...
                        <div className="flex flex-col gap-2">
                            {uris.map(uri => (
                                <div key={uri} className="flex items-start gap-2">
                                    <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-indigo-400 shrink-0" />
                                    <p className="text-sm text-grayscale-700 font-mono break-all">{uri}</p>
                                </div>
                            ))}
                        </div>
                    </div>
