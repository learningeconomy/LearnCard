...
export const AiSessionTopicItem: React.FC<{
    ...
    const appLogos = [app?.img, topicVc?.image].filter(Boolean) as string[];
    ...
    return (
        <button
            ...
            onClick={() => {
                ...
                history.push(`/ai/sessions?topicBoostUri=${encodeURIComponent(topicBoost?.uri ?? '')}`);
            }}
        ...
        >
            <div className="flex items-center justify-start flex-1 min-w-0">
                <div className="flex items-center">
                    {uniqueLogos.length > 0 ? (
                        uniqueLogos.map((logo, idx) => (
                            <img
                                key={`${logo}-${idx}`}
                                className={`w-[34px] h-[34px] object-cover bg-white rounded-[10px] overflow-hidden border border-grayscale-200 ${idx > 0 ? '-ml-2' : ''}`}
                                alt="AI tool logo"
                                src={logo}
                            />
                        ))
            ...
            <div className="flex items-center justify-end gap-1 shrink-0 ml-3 text-grayscale-600 font-poppins text-sm">
                {hasUnfinishedSessions && <span className="w-[8px] h-[8px] rounded-full bg-rose-500 shrink-0" />}
