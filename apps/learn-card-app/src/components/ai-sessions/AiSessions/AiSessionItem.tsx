...
export const AiSessionItem: React.FC<{
...
}> = ({ app, session, topicRecord, topicVc, handleTopicSession, selectedSession, forceSingleView }) => {
    ...
    const subtitleApp = session?.record?.app || '';
    ...
    return (
        <button
            ...
            className={`flex items-center justify-between w-full bg-white rounded-2xl shadow-sm px-4 py-3 mb-3 ${isSelectedSession ? 'ring-2 ring-cyan-200' : ''}`}
        >
            <div className="flex items-center justify-start w-full min-w-0">
                <div className="flex flex-col items-start w-full min-w-0">
                    <p className="w-full text-grayscale-900 text-[24px] leading-tight font-poppins font-semibold text-left line-clamp-1">
                        {getAiSessionTitle(sessionCredential as any) || 'Untitled Session'}
                    </p>
                    <p className="text-grayscale-600 text-sm font-poppins text-left line-clamp-1">
                        ...
                        <span className={isCompleted ? 'text-grayscale-600' : 'text-rose-500'}>{dateLabel}</span>
                    </p>
                </div>
            </div>

            <span className="flex items-center justify-end text-grayscale-600 font-poppins text-sm ml-2 shrink-0">
                {isLoading ? (
