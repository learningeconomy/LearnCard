...
type CategoryAndTypeSelectorProps = {
...
const BoostCMSCategoryAndTypeSelector: React.FC<CategoryAndTypeSelectorProps> = ({
    activeCategoryType,
    activeType,
    setActiveType,
    customTypes,
    ...
    if (activeCategoryType === BoostCategoryOptionsEnum.accomplishment) {
    ...
    return (
        <IonGrid
            className={`w-full h-full ion-no-padding bg-${color} flex justify-center items-start overflow-y-auto`}
        >
            <IonRow
            ...
            >
                <div className="w-full ion-padding bg-white rounded-tr-[20px] rounded-tl-[20px] mb-0 relative">
                    <IonInput
                        ...
                        placeholder={m['boost.cms.appearance.search']()}
                        type="text"
                    ...
                    {filteredAchievementTypes.length === 0 && (
                        <div className="w-full text-left flex flex-col items-start justify-center">
                            <p className="text-grayscale-600 text-base font-normal">
                                {m['boost.cms.appearance.noResultsFor']()}{' '}
                            ...
                            >
                                {m['boost.cms.appearance.createBoost']({ search })}
                            </button>
                        </div>
                    )}
                    {search.length > 0 &&
                        filteredAchievementTypes.map(({ title, type }) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative flex items-center justify-between  rounded-full px-[10px] py-[10px] w-full shadow-lg normal tracking-wide max-w-[95%] mb-4 ${activeStyles}`}
                                >
                                    <div
                                        className={`flex items-center justify-center h-[50px] w-[50px] rounded-full ${bgColor} border-white border-2 overflow-hidden`}
                                    >
                                        <img
                                            src={CategoryImage}
                                            alt="category"
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-start pl-4">
                                        {isCustomType && (
                                            <p className="w-full text-gray-800 text-xs normal font-bold text-left">
                                                {m['boost.cms.appearance.customType']()}
                                            </p>
                                        )}
