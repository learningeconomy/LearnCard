...
type SkillProficiencyBarProps = {
    ...
    mode?: SkillProficiencyBarModeEnum;
    onChange?: (level: SkillLevel) => void;
};

const SkillProficiencyBar: React.FC<SkillProficiencyBarProps> = ({
    proficiencyLevel,
    mode = SkillProficiencyBarModeEnum.Slider,
    onChange,
}) => {
    ...
    const isDisplayMode = mode === SkillProficiencyBarModeEnum.Display;
    ...
    useEffect(() => {
        if (!isDragging || isDisplayMode) return;
    ...
    return (
        <div className={`flex flex-col w-full ${isDisplayMode ? 'gap-[10px]' : 'gap-[15px]'}`}>
            {isDisplayMode && (
            ...
            >
                {LEVELS.map((level, index) => {
                    if (isDisplayMode && level === SkillLevel.Hidden) return null;

                    const displayStyles = 'cursor-default';
                    const sliderStyles = `h-[29px] py-[11px] ${
                    ...
                    return (
                        <div
                            ...
                            className={`flex-1 relative ${
                                isDisplayMode ? displayStyles : sliderStyles
                            }`}
