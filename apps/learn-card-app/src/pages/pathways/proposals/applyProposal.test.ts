...
import { ProposalApplyError, applyProposal, materializeNewPathway } from './applyProposal';
...
const proposal = (diff: PathwayDiff, overrides: Partial<Proposal> = {}): Proposal => ({
    id: 'prop-1',
    pathwayId: 'p1',
    ownerDid: 'did:test:learner',
    agent: 'planner',
...
describe('applyProposal — in-pathway diffs', () => {
    it('adds new nodes and stamps pathwayId on them', () => {
        ...
        const after = applyProposal(before, proposal(emptyDiff({ addNodes: [addition] })), LATER);
    ...
    it('removes nodes and prunes any edge referencing them', () => {
        ...
        const after = applyProposal(before, proposal(emptyDiff({ removeNodeIds: ['b'] })), LATER);
    ...
    it('rejects additions whose id collides with an existing node', () => {
        ...
        expect(() => applyProposal(before, proposal(emptyDiff({ addNodes: [node('a')] })))).toThrow(
            ProposalApplyError
        );
    });
    ...
    it('rejects new edges whose endpoints do not exist in the result', () => {
        ...
        expect(() =>
            applyProposal(before, proposal(emptyDiff({ addEdges: [edge('e1', 'a', 'ghost')] })))
        ).toThrow(ProposalApplyError);
    });
    ...
    it('refuses to apply a proposal that targets a different pathway', () => {
        ...
        expect(() =>
            applyProposal(before, proposal(emptyDiff(), { pathwayId: 'other-pathway' }))
        ).toThrow(ProposalApplyError);
    });
    ...
    it('is pure — original pathway is not mutated', () => {
        ...
        applyProposal(before, proposal(emptyDiff({ addNodes: [node('b')] })), LATER);
    ...
    it('preserves chosenRoute when the diff removes nothing on it', () => {
        ...
        const after = applyProposal(before, proposal(emptyDiff({ addNodes: [node('d')] })), LATER);
    ...
    it('drops chosenRoute entirely when pruning leaves fewer than two ids', () => {
        ...
        // "route."
        const before = pathway([node('a'), node('b')], [], { chosenRoute: ['a', 'b'] });
    ...
    it('leaves chosenRoute absent when it was absent to begin with', () => {
        ...
        const after = applyProposal(before, proposal(emptyDiff({ addNodes: [node('c')] })), LATER);
    ...
    it('an empty setChosenRoute clears the field (honors the learner intent)', () => {
        ...
        const after = applyProposal(before, proposal(emptyDiff({ setChosenRoute: [] })), LATER);
...
describe('materializeNewPathway — cross-pathway proposals', () => {
    ...
    it('throws when the proposal is cross-pathway but lacks a newPathway hint', () => {
        expect(() =>
            materializeNewPathway(proposal(emptyDiff(), { pathwayId: null }), {
                ownerDid: OWNER,
                now: LATER,
            })
        ).toThrow(ProposalApplyError);
...
describe('applyProposal — composite guard', () => {
    ...
    it('rejects a composite ref that closes a cross-pathway cycle', () => {
        ...
        expect(() =>
            applyProposal(
                a,
                proposal(emptyDiff({ addNodes: [compositeNode('n-a', REF_B)] }), {
                    pathwayId: REF_A,
                }),
                LATER,
    ...
    it('accepts a composite ref that does NOT create a cycle', () => {
        ...
        const after = applyProposal(
            a,
            proposal(emptyDiff({ addNodes: [compositeNode('n-a', REF_B)] }), { pathwayId: REF_A }),
            LATER,
            { allPathways: { [REF_A]: a, [REF_B]: b } }
        );
...
describe('applyProposal — outcome bindings', () => {
    ...
    it('applies multiple binding patches in one proposal', () => {
        ...
        const before = pathway([], [], {
            outcomes: [satOutcome(), satOutcome({ id: OUTCOME_ID_B, label: 'Other' })],
        });
