...
export const applyProposal = (
    ...
    for (const patch of diff.updateNodes) {
        if (!nodeIdSet.has(patch.id)) {
            throw new ProposalApplyError(`Proposal patches unknown node ${patch.id}`);
        }
    }
    ...
    for (const edge of diff.addEdges) {
        if (!nextNodeIdSet.has(edge.from) || !nextNodeIdSet.has(edge.to)) {
            throw new ProposalApplyError(`Proposal adds edge ${edge.id} with unknown endpoint`);
        }
    }
    ...
    // swapping routes).
    const sourceRoute =
        diff.setChosenRoute !== undefined ? diff.setChosenRoute : pathway.chosenRoute;
    ...
    const nextPathway: Pathway = {
        ...
        // actively strip it.
        ...(nextChosenRoute ? { chosenRoute: nextChosenRoute } : { chosenRoute: undefined }),
