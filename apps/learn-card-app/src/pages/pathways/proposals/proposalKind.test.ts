...
describe('proposalKind', () => {
    ...
    it('classifies a diff with only setChosenRoute as route-swap', () => {
        expect(proposalKind(emptyDiff({ setChosenRoute: ['a', 'b'] }))).toBe('route-swap');
    });
    ...
    it('classifies a diff with an empty setChosenRoute (clearing route) as route-swap', () => {
        ...
        expect(proposalKind(emptyDiff({ setChosenRoute: [] }))).toBe('route-swap');
    });

    it('classifies a diff with added nodes as pathway-edit', () => {
        expect(proposalKind(emptyDiff({ addNodes: [fakeNode()] }))).toBe('pathway-edit');
    });

    it('classifies a diff with removed nodes as pathway-edit', () => {
        expect(proposalKind(emptyDiff({ removeNodeIds: ['x'] }))).toBe('pathway-edit');
    });
    ...
    it('classifies a diff with updated nodes as pathway-edit', () => {
        expect(
            proposalKind(
                emptyDiff({
                    updateNodes: [{ id: '00000000-0000-4000-8000-000000000005', title: 'new' }],
                })
            )
        ).toBe('pathway-edit');
    });
    ...
    it('classifies a diff with edge changes as pathway-edit', () => {
        expect(proposalKind(emptyDiff({ addEdges: [fakeEdge()] }))).toBe('pathway-edit');

        expect(proposalKind(emptyDiff({ removeEdgeIds: ['e1'] }))).toBe('pathway-edit');
    });
...
describe('PROPOSAL_KIND_FRAMING', () => {
    it('provides framing copy for every ProposalKind', () => {
        const kinds = ['route-swap', 'pathway-edit', 'mixed', 'new-pathway', 'empty'] as const;
