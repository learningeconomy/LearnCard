...
const terminationSummary = (termination: Termination): string => {
    switch (termination.kind) {
        ...
        case 'endorsement':
            return `${termination.minEndorsers} endorsement${
                termination.minEndorsers === 1 ? '' : 's'
            }`;
        case 'self-attest':
            return 'self-attest';
...
const ProposalDiff: React.FC<ProposalDiffProps> = ({ diff, pathway }) => {
    ...
    if (nothingToShow) {
        return (
            <p className="text-xs text-grayscale-500 italic">This proposal contains no changes.</p>
        );
    }
