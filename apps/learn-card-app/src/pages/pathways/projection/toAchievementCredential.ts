...
import type { AchievementProjection, EndorsementRef, Evidence, PathwayNode } from '../types';
...
export const toAchievementCredential = ({
    ...
    if (!node.credentialProjection) {
        throw new ProjectionError(`Node ${node.id} has no credentialProjection; cannot project.`);
    }
