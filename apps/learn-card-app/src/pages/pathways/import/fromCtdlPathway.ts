...
import type { Cadence, Edge, Evidence, Pathway, PathwayNode, Policy, Termination } from '../types';
...
export const fromCtdlPathway = (opts: FromCtdlOptions): FromCtdlResult => {
    ...
    // ---------------------------------------------------------------
    const destinationRefs = extractRefIds(graph.pathway['ceterms:hasDestinationComponent']);

    let destinationNodeId: string | undefined;

    if (destinationRefs.length === 0) {
        warnings.push('No destination component found — this pathway has no terminal node.');
    } else {
    ...
    // ---------------------------------------------------------------
    const title =
        getLocalizedString(graph.pathway['ceterms:name'], preferredLocale) || 'Imported pathway';

    const description = getLocalizedString(graph.pathway['ceterms:description'], preferredLocale);
...
const buildNode = ({
    ...
    const title =
        getLocalizedString(component['ceterms:name'], preferredLocale) || '(untitled component)';

    const description = getLocalizedString(component['ceterms:description'], preferredLocale);
    ...
    const node: PathwayNode = {
        ...
        sourceCtid: component['ceterms:ctid'] ?? extractCtidFromUri(component['@id']),
    };
...
const pickStageDefaults = (
    ...
    const title = getLocalizedString(component['ceterms:name']) || '(untitled)';
    ...
    switch (type) {
        case 'ceterms:CredentialComponent':
            return {
                policy: {
                    kind: 'artifact',
                    prompt: 'Earn this credential and attach proof (a link to the badge, certificate PDF, or similar).',
        ...
        case 'ceterms:CourseComponent':
            return {
                policy: {
                    kind: 'artifact',
                    prompt: 'Complete this course and attach the completion link or certificate.',
                    expectedArtifact: 'link',
                },
                termination: {
                    kind: 'artifact-count',
        ...
        case 'ceterms:CompetencyComponent':
            return {
                ...
                termination: {
                    kind: 'self-attest',
                    prompt: 'I have demonstrated this competency with concrete evidence.',
                },
            };
...
const getEffectiveComponent = (
    ...
    if (!merged['ceterms:subjectWebpage'] && typeof proxy['ceterms:subjectWebpage'] === 'string') {
        merged['ceterms:subjectWebpage'] = proxy['ceterms:subjectWebpage'];
    }

    if (!merged['ceterms:sourceData'] && typeof proxy['ceterms:sourceData'] === 'string') {
        merged['ceterms:sourceData'] = proxy['ceterms:sourceData'];
    }

    if (!merged['ceterms:image'] && typeof proxy['ceterms:image'] === 'string') {
    ...
    if (!merged['ceterms:description'] && proxy['ceterms:description'] !== undefined) {
 ...
 */
const buildProjection = (component: CtdlPathwayComponent, title: string, description: string) => {
    const type = String(component['@type']);
    ...
    const achievementType = (() => {
        ...
        if (rawType) {
            ...
            if (typeString) {
                ...
                const cleaned = typeString.replace(/^ceterms:/, '').replace(/^.*\//, '');
