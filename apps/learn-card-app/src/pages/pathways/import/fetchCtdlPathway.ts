...
export const DEFAULT_REGISTRY_BASE = 'https://credentialengineregistry.org/resources';
 ...
 */
export const fetchCtdlPathway = async (opts: FetchCtdlPathwayOptions): Promise<CtdlGraph> => {
    const {
        ctidOrUrl,
...
const normalizeToUrl = (ctidOrUrl: string, registryBase: string): string => {
    ...
    throw new CtdlFetchError(`\`ctidOrUrl\` "${ctidOrUrl}" is neither a CTID nor an http(s) URL.`);
};
...
const fetchJson = async <T>(url: string, fetchImpl: typeof fetch): Promise<T> => {
    ...
    try {
        ...
        throw new CtdlFetchError(`Network error while fetching CTDL resource ${url}.`, err);
    }
    ...
    try {
        ...
        throw new CtdlFetchError(`CTDL resource ${url} returned invalid JSON.`, err);
    }
};
