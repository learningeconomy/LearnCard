...
describe('ctdlTypes helpers', () => {
    describe('getLocalizedString', () => {
        ...
        it('prefers the exact preferred locale', () => {
            expect(getLocalizedString({ 'en-US': 'Hi', 'es': 'Hola' }, 'en-US')).toBe('Hi');
        });

        it('falls back to a prefix match when the exact locale is missing', () => {
            expect(getLocalizedString({ 'en-GB': 'Howdy', 'es': 'Hola' }, 'en-US')).toBe('Howdy');
        });
    ...
    describe('extractCtidFromUri', () => {
        it('pulls the CTID out of a registry URL', () => {
            expect(
                extractCtidFromUri('https://credentialengineregistry.org/resources/ce-abc-123')
            ).toBe('ce-abc-123');
        });
    ...
    describe('extractRefIds — real CTDL shapes', () => {
        it('extracts URIs from an array of bare strings', () => {
            expect(extractRefIds(['https://x.test/ce-a', 'https://x.test/ce-b'])).toEqual([
                'https://x.test/ce-a',
                'https://x.test/ce-b',
            ]);
        });
        ...
        it('extracts URIs from an array of object refs', () => {
            expect(
                extractRefIds([{ '@id': 'https://x.test/ce-a' }, { '@id': 'https://x.test/ce-b' }])
            ).toEqual(['https://x.test/ce-a', 'https://x.test/ce-b']);
        });

        it('extracts URIs from a mixed-form array', () => {
            expect(
                extractRefIds(['https://x.test/ce-a', { '@id': 'https://x.test/ce-b' }])
            ).toEqual(['https://x.test/ce-a', 'https://x.test/ce-b']);
        });

        it('handles a single bare-string ref (not wrapped in an array)', () => {
            expect(extractRefIds('https://x.test/ce-solo')).toEqual(['https://x.test/ce-solo']);
        });
    });
    ...
    describe('getPathwayMemberRefs', () => {
        it('unions hasPart and hasChild, deduped', () => {
            const refs = getPathwayMemberRefs({
                ...
                'ceterms:hasPart': ['https://x.test/ce-a', 'https://x.test/ce-b'],
                'ceterms:hasChild': [{ '@id': 'https://x.test/ce-b' }],
            });

            expect(refs).toEqual(['https://x.test/ce-a', 'https://x.test/ce-b']);
        });
...
describe('fromCtdlPathway — linear pathway', () => {
    ...
    it('builds prerequisite edges following hasChild ordering', () => {
        const { pathway } = runImport();
        const titleOf = (id: string) => pathway.nodes.find(n => n.id === id)?.title ?? '(missing)';
...
describe('fromCtdlPathway — stage defaults', () => {
    ...
    it('picks a practice policy + self-attest termination for CompetencyComponent', () => {
        ...
        const competency = pathway.nodes.find(n => n.title === 'Demonstrate Scope Planning')!;
...
describe('fromCtdlPathway — earn-link capture', () => {
    ...
    const buildGraph = (componentOverrides: Record<string, unknown>) => ({
        pathway: {
            '@id': 'https://x.test/ce-root',
            '@type': 'ceterms:Pathway',
    ...
    it('captures `ceterms:sourceData` as the primary earn URL', () => {
        ...
        expect(dest?.credentialProjection?.earnUrl).toBe('https://issuer.example/earn');
    ...
    it('falls back to `ceterms:subjectWebpage` when `sourceData` is absent', () => {
        ...
        expect(dest?.credentialProjection?.earnUrl).toBe('https://issuer.example/about');
        expect(dest?.credentialProjection?.earnUrlSource).toBe('subjectWebpage');
    });
...
describe('fromCtdlPathway — proxyFor resolution', () => {
    ...
    it("uses the proxied credential's `ceterms:sourceData` when present (priority 1)", () => {
        ...
        expect(dest?.credentialProjection?.earnUrl).toBe('https://issuer.example/enroll');
        expect(dest?.credentialProjection?.earnUrlSource).toBe('sourceData');
    });
    ...
    it("captures the proxied credential's `ceterms:image` into the projection", () => {
        ...
        expect(dest?.credentialProjection?.image).toBe('https://images.example/badge.png');
    });
    ...
    it('lets the component win when both component and proxy declare the same field', () => {
        ...
        expect(dest?.credentialProjection?.earnUrl).toBe('https://authors.choice/landing');
    });
...
describe('fromCtdlPathway — image capture', () => {
    ...
    it('captures `ceterms:image` directly from the component when present', () => {
        ...
        expect(dest?.credentialProjection?.image).toBe('https://images.example/component.png');
    });
