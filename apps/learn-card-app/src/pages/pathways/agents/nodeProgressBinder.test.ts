...
describe('bindEventToNodes — credential-ingested × requirement-satisfied', () => {
    ...
    it('does not record a skip for nodes with unrelated terminations', () => {
        ...
        const relevant = nodeWithTermination('00000000-0000-4000-8000-000000000006', {
            kind: 'requirement-satisfied',
            requirement: { kind: 'credential-type', type: 'AWSCertifiedCloudPractitioner' },
            minTrustTier: 'trusted',
        });

        const unrelated = nodeWithTermination('00000000-0000-4000-8000-000000000007', {
            kind: 'self-attest',
            prompt: 'I did it',
        });
...
describe('bindEventToNodes — cross-pathway', () => {
    it('emits a proposal per matching node across pathways', () => {
        ...
        const pathwayA = pathwayWith([node1], '00000000-0000-4000-8000-aaaaaaaaaaa1');
        const pathwayB = pathwayWith([node2], '00000000-0000-4000-8000-bbbbbbbbbbb2');
        ...
        expect(proposals.map(p => p.pathwayId).sort()).toEqual([pathwayA.id, pathwayB.id].sort());
    });
});
