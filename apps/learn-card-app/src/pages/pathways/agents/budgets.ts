...
export const decideBudget = (state: BudgetState): BudgetDecision => {
    ...
    if (state.learnerMonthToDateCents + state.projectedCostCents > LEARNER_MONTHLY_CAP_CENTS) {
        return {
            allowed: false,
    ...
    if (state.tenantMonthToDateCents + state.projectedCostCents > TENANT_MONTHLY_CAP_CENTS) {
        return {
            allowed: false,
