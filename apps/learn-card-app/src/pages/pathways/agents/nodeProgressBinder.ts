...
import { classifyIssuerTrust, type IssuerTrustRegistry, type VcLike } from '../core/outcomeMatcher';
import type {
...
const reasonStringFor = (event: WalletEvent, node: PathwayNode, pathway: Pathway): string =>
    ...
    for (const pathway of request.pathways) {
        for (const node of pathway.nodes) {
            ...
            if (node.progress.status === 'completed') {
                ...
                if (isApplicableTerminationFor(node.stage.termination, request.event)) {
                    skipped.push({
                        pathwayId: pathway.id,
                        nodeId: node.id,
 ...
 */
const isApplicableTerminationFor = (termination: Termination, event: WalletEvent): boolean => {
