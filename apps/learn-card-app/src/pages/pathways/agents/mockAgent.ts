...
import type { AgentName, Capability, Pathway, PathwayDiff, PathwayNode, Proposal } from '../types';
...
const makeInterpretationProposal = ({ pathway, now, ownerDid }: MockAgentRequest): Proposal => {
    if (!pathway) {
...
const makePlanningProposal = ({ pathway, now, ownerDid }: MockAgentRequest): Proposal => {
    if (!pathway) {
...
const makeNudgingProposal = ({ pathway, now, ownerDid }: MockAgentRequest): Proposal => {
    if (!pathway) {
...
const makeRoutingProposal = ({ pathway, now, ownerDid }: MockAgentRequest): Proposal => {
    if (!pathway) {
    ...
    const diff: PathwayDiff = {
        ...emptyDiff(),
        addNodes: [alt],
        addEdges: target ? [{ id: uuid(), from: target.id, to: alt.id, type: 'alternative' }] : [],
    };
...
const makeMatchingProposal = ({ now, ownerDid }: MockAgentRequest): Proposal => ({
    id: uuid(),
    pathwayId: null,
    ownerDid,
