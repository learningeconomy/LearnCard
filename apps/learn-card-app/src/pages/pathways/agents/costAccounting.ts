...
export const monthToDateCents = (history: readonly InvocationEvent[], now: string): number => {
    const key = monthKey(now);

    return history.reduce((sum, e) => (monthKey(e.at) === key ? sum + e.costCents : sum), 0);
};
...
export const dailyInvocationsFor = (
    ...
    return history.reduce(
        (count, e) =>
            e.capability === capability && new Date(e.at).getTime() >= cutoff ? count + 1 : count,
        0
    );
};
