...
const event = (at: string, capability: Capability, costCents: number): InvocationEvent => ({
    capability,
    costCents,
    at,
});
...
describe('recordInvocation', () => {
    it('appends the event and leaves recent history intact', () => {
        const prior: InvocationEvent[] = [event('2026-04-01T00:00:00.000Z', 'planning', 5)];

        const next = recordInvocation(
            prior,
            event('2026-04-20T00:00:00.000Z', 'interpretation', 1)
        );
...
describe('buildCostSnapshot', () => {
    it('matches the payload shape consumed by pathways.learnerCost.snapshot', () => {
        ...
        const snapshot = buildCostSnapshot(history, 'did:key:zlearner', '2026-04-20T12:00:00.000Z');
