...
describe('seedChosenRoute', () => {
    ...
    it('routes a linear chain from entry to destination', () => {
        ...
        const pathway = makePathway([a, b, c], [edge(a.id, b.id), edge(b.id, c.id)], c.id);
...
describe('pruneChosenRoute', () => {
    ...
    it('drops ids that no longer survive', () => {
        ...
        expect(pruneChosenRoute(['a', 'b', 'c', 'd'], surviving)).toEqual(['a', 'c', 'd']);
    });
    ...
    it('preserves order', () => {
        ...
        expect(pruneChosenRoute(['a', 'b', 'c', 'd'], surviving)).toEqual(['a', 'b', 'd']);
    });
...
describe('pickNextOnRoute', () => {
    ...
    it('returns null when no route node is both uncompleted and available', () => {
        ...
        expect(pickNextOnRoute(allDone, [a.id, b.id, c.id])).toBeNull();
    });
