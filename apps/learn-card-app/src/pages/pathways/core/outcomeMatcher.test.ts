...
const scoreThreshold = (overrides: Partial<ScoreThresholdSignal> = {}): ScoreThresholdSignal => ({
    id: OUTCOME_ID,
...
describe('matchVcAgainstOutcome — score-threshold', () => {
    ...
    it('accepts dot-paths without a leading credentialSubject prefix', () => {
        ...
        expect(matchVcAgainstOutcome(scoreThreshold({ field: 'score.total' }), vc)).toMatchObject({
            matched: true,
            observedValue: 1500,
        });
    });
    ...
    it('walks multi-subject VCs until it finds the field', () => {
        const vc: VcLike = {
            type: ['CollegeBoardSATScore'],
            credentialSubject: [{ id: 'did:example:alice' }, { score: { total: 1450 } }],
        };
...
describe('checkWindow', () => {
    ...
    it('passes when the VC has no issuance date', () => {
        expect(checkWindow(base, {}, { createdAt: '2026-01-01T00:00:00Z' })).toEqual({
            inWindow: true,
        });
    });
    ...
    it('marks in-window VCs correctly', () => {
        ...
        expect(checkWindow(base, vc, { createdAt: '2026-01-01T00:00:00Z' })).toEqual({
            inWindow: true,
        });
    });
    ...
    it('treats durationDays=0 as unbounded from the start', () => {
        ...
        expect(checkWindow(signal, vc, { createdAt: '2026-01-01T00:00:00Z' })).toEqual({
            inWindow: true,
        });
    });
});
