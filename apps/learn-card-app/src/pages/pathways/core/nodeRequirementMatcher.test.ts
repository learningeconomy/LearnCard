...
const baseIdentity = (overrides: Partial<CredentialIdentity> = {}): CredentialIdentity => ({
    types: [],
    issuerDid: null,
    credentialId: null,
    raw: {},
...
describe('matchRequirement — boost-uri leaf', () => {
    it('exact-matches the canonical wallet boostUri', () => {
        ...
        expect(
            matchRequirement({ kind: 'boost-uri', uri: 'boost:aws-iam-deep-dive' }, identity)
                .matched
        ).toBe(true);
    });
    ...
    it('fails with boost-uri-mismatch when the URI is absent or different', () => {
        ...
        expect(
            matchRequirement({ kind: 'boost-uri', uri: 'boost:anything' }, baseIdentity())
        ).toEqual({ matched: false, reason: 'boost-uri-mismatch' });
    });
});
...
describe('matchRequirement — ctdl-ctid leaf', () => {
    ...
    it('matches case-insensitively', () => {
        ...
        expect(
            matchRequirement({ kind: 'ctdl-ctid', ctid: ctid.toUpperCase() }, identity).matched
        ).toBe(true);
    });
...
describe('matchRequirement — skill-tag leaf', () => {
    it('matches exact first, case-insensitive fallback second', () => {
        ...
        const exact = matchRequirement({ kind: 'skill-tag', tag: 'IAM' }, identity);
        const caseFold = matchRequirement({ kind: 'skill-tag', tag: 'cross-account' }, identity);
...
describe('firstSatisfying', () => {
    it('returns the first matching requirement index, or -1', () => {
        ...
        expect(firstSatisfying([{ kind: 'credential-type', type: 'DoesNotExist' }], identity)).toBe(
            -1
        );
    });
});
