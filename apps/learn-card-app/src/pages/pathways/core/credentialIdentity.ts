...
const extractCtdlCtid: IdentityExtractor = vc => {
    ...
    for (const subject of subjectObjects(vc)) {
        candidates.push(subject.ctid, subject.sourceCtid, subject.sourceUri, subject.id);

        const achievement = subject.achievement;
        const record = Array.isArray(achievement)
            ? asRecord(achievement[0])
            : asRecord(achievement);

        if (record) {
            candidates.push(record.ctid, record.sourceCtid, record.sourceUri, record.id);
        }
    }
