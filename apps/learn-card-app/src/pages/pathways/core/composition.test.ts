...
describe('wouldCreateCycle', () => {
    ...
    it('permits embedding a pathway that does not reach back', () => {
        ...
        expect(
            wouldCreateCycle({ [parent.id]: parent, [child.id]: child }, parent.id, child.id)
        ).toBe(false);
    });
    ...
    it('flags a transitive back-reference (A ← B ← C, adding A→C closes the loop)', () => {
        ...
        expect(wouldCreateCycle({ [a.id]: a, [b.id]: b, [c.id]: c }, a.id, c.id)).toBe(true);
    });
    ...
    it('returns false when the candidate child is not in the map at all', () => {
        ...
        expect(wouldCreateCycle({ [parent.id]: parent }, parent.id, UUID(99))).toBe(false);
    });
});
