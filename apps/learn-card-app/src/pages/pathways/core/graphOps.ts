...
const traverse = (start: string, adjacency: Map<string, Set<string>>): Set<string> => {
 ...
 */
export const prereqProgress = (pathway: Pathway, nodeId: string): PrereqProgress => {
    const { prereqs } = buildAdjacency(pathway);
 ...
 * docs § 10.
 */
export const neighborhood = (pathway: Pathway, focusId: string, radius: number): Neighborhood => {
    ...
    const edgeIds = new Set(
        pathway.edges.filter(e => nodeIds.has(e.from) && nodeIds.has(e.to)).map(e => e.id)
    );
