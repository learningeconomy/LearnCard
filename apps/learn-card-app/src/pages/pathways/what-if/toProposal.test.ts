...
describe('classifyScenarioForProposal', () => {
    it('classifies an effort-multiplier scenario as unsupported', () => {
        ...
        expect(classifyScenarioForProposal(pathway, deepPracticeScenario()).kind).toBe(
    ...
    it('classifies an empty selector as empty-scenario', () => {
        ...
        expect(classifyScenarioForProposal(pathway, empty).kind).toBe('empty-scenario');
    });
    ...
    it('classifies a pure-skip scenario with no matching nodes on the route as no-effect', () => {
        ...
        const pathway = makePathway([a, b], [edge(a.id, b.id)], b.id, [a.id, b.id]);

        expect(classifyScenarioForProposal(pathway, fastTrackScenario()).kind).toBe('no-effect');
    });
    ...
    it('classifies a pure-skip scenario with matching route nodes as ok', () => {
        ...
        const pathway = makePathway([p, r, a], [edge(p.id, r.id), edge(r.id, a.id)], a.id, [
            p.id,
            r.id,
            a.id,
        ]);

        expect(classifyScenarioForProposal(pathway, fastTrackScenario()).kind).toBe('ok');
    });
    ...
    it('classifies a mixed multiplier + skip scenario as unsupported', () => {
        ...
        expect(classifyScenarioForProposal(pathway, mixed).kind).toBe('multiplier-unsupported');
    });
});
...
describe('computeTargetRoute', () => {
    ...
    it('filters the pathway chosenRoute against skipIds, preserving order', () => {
        ...
        expect(computeTargetRoute(pathway, fastTrackScenario())).toEqual([p.id, a.id]);
    });
    ...
    it('returns null when the target would be fewer than two nodes', () => {
        ...
        const pathway = makePathway([p, r], [edge(p.id, r.id)], r.id, [p.id, r.id]);
...
describe('buildScenarioDiff', () => {
    it('returns null for unsupported scenarios', () => {
        ...
        expect(buildScenarioDiff(pathway, deepPracticeScenario(), makeIds())).toBeNull();
    });
    ...
    it('returns null when no route nodes match the skip filter', () => {
        ...
        expect(buildScenarioDiff(pathway, fastTrackScenario(), makeIds())).toBeNull();
    });
    ...
    it('emits a non-structural diff that only sets chosenRoute', () => {
        ...
        const diff = buildScenarioDiff(pathway, fastTrackScenario(), makeIds())!;
    ...
    it('produces a diff applyProposal consumes into a route swap with intact graph', () => {
        ...
        const proposal = buildProposalFromScenario(pathway, fastTrackScenario(), [], {
            ownerDid: OWNER,
            now: NOW,
            makeId: makeIds(),
        })!;
...
describe('buildProposalFromScenario', () => {
    const pathwayWithReview = () => {
        ...
        return makePathway([p, r, a], [edge(p.id, r.id), edge(r.id, a.id)], a.id, [
            p.id,
            r.id,
            a.id,
        ]);
    ...
    it('sets an expiry honoring the ttlDays option', () => {
        ...
        const expected = new Date(new Date(NOW).getTime() + 3 * 24 * 60 * 60 * 1000).toISOString();
    ...
    it('carries the scenario authored tradeoffs on the proposal', () => {
        ...
        const difficulty = proposal.tradeoffs?.find(t => t.dimension === 'difficulty');
    ...
    it('prefers caller-passed tradeoffs over authored ones on the same dimension', () => {
        ...
        expect(difficulty?.deltaDescription).toBe('Live-measured: retention drop is real');

        expect(proposal.tradeoffs?.some(t => t.dimension === 'time')).toBe(true);
    });

    it('produces a deterministic proposal id under an injected makeId', () => {
        const pathway = pathwayWithReview();

        const a = buildProposalFromScenario(pathway, fastTrackScenario(), [], {
            ownerDid: OWNER,
            now: NOW,
            makeId: makeIds(),
        });
        const b = buildProposalFromScenario(pathway, fastTrackScenario(), [], {
            ownerDid: OWNER,
            now: NOW,
            makeId: makeIds(),
        });
