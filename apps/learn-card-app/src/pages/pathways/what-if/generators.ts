 ...
 * already done.
 */
const countUncompletedByKind = (pathway: Pathway): Record<Policy['kind'], number> => {
...
const fastTrack = (pathway: Pathway): Scenario | null => {
    ...
    return {
        ...
        kind: 'fast-track',
        title: 'Fast track',
        subtitle: `Skip ${counts.review} review step${
            counts.review === 1 ? '' : 's'
...
const deepPractice = (pathway: Pathway): Scenario | null => {
    ...
    return {
        ...
        kind: 'deep-practice',
        title: 'Deep practice',
        subtitle: `Double the practice load across ${counts.practice} session${
            counts.practice === 1 ? '' : 's'
        }`,
        authoredTradeoffs,
...
const externalLight = (pathway: Pathway): Scenario | null => {
    ...
    return {
        ...
        subtitle: `Skip ${counts.external} external module${
            counts.external === 1 ? '' : 's'
...
const compositeBypass = (pathway: Pathway): Scenario | null => {
    ...
    return {
        ...
        subtitle: `Bypass ${counts.composite} nested sub-pathway${
            counts.composite === 1 ? '' : 's'
        } and stay on the main track`,
        authoredTradeoffs,
...
const destinationOnly = (pathway: Pathway): Scenario | null => {
    ...
    return {
        ...
        subtitle: `Run the shortest path to the destination and set ${skipIds.length} sibling step${
            skipIds.length === 1 ? '' : 's'
        } aside`,
        authoredTradeoffs,
