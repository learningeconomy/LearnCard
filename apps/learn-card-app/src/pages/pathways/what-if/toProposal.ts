...
export const classifyScenarioForProposal = (
    ...
    const hasSkipKinds =
        selector.skipPolicyKinds !== undefined && selector.skipPolicyKinds.length > 0;

    const hasSkipIds = selector.skipNodeIds !== undefined && selector.skipNodeIds.length > 0;
    ...
    if (!targetRoute) {
        return {
            kind: 'no-effect',
            message: 'Nothing on your route matches the scenario\u2019s filters today.',
        };
    }
 ...
 */
export const resolveSkipIds = (pathway: Pathway, scenario: Scenario): Set<string> => {
    const { selector } = scenario;
 ...
 */
export const computeTargetRoute = (pathway: Pathway, scenario: Scenario): string[] | null => {
