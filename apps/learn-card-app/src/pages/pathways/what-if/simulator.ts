...
import type { NodeSelector, Scenario, ScenarioResult, SimulationSummary } from './types';
 ...
 */
const effortMultiplier = (policyKind: Policy['kind'], selector: NodeSelector): number => {
...
export const simulate = (
    ...
    for (const id of zone) {
        ...
        const mins = nodeEffortMinutes(node) * effortMultiplier(node.stage.policy.kind, selector);
 ...
 */
const timeTradeoffFromDelta = (etaDelta: number | null): Tradeoff | null => {
    ...
    const absText =
        absMin < 60 ? `${Math.round(absMin)} min` : `${Math.round((absMin / 60) * 10) / 10} hr`;

    return {
        dimension: 'time',
        deltaDescription:
            etaDelta < 0 ? `About ${absText} less of work` : `About ${absText} more of work`,
 ...
 */
export const simulateScenario = (pathway: Pathway, scenario: Scenario): ScenarioResult => {
 ...
 */
export const simulateAll = (pathway: Pathway, scenarios: readonly Scenario[]): ScenarioResult[] =>
    scenarios.map(s => simulateScenario(pathway, s));
