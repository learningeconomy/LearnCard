...
export type Greeting = 'Good morning' | 'Good afternoon' | 'Good evening' | 'Still here';
 ...
 * unit-testable.
 */
export const resolvePolicyCallToAction = (policy: Policy, mcpLabel?: string | null): string => {
 ...
 */
export const resolveNodeCallToAction = (node: PathwayNode, mcpLabel?: string | null): string => {
    ...
    if (projection) {
        ...
        if (termination.kind === 'assessment-score' || policy.kind === 'assessment') {
        ...
        if (termination.kind === 'artifact-count' || policy.kind === 'artifact') {
 ...
 */
export const buildIdentityBanner = (goal: string, altitude?: Altitude): IdentityBannerContent => {
    const trimmed = goal.trim();
