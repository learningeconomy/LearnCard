...
import { computeProgressTier, humanizeNodeTitle, resolveCtaCopy } from './progressCtaCopy';
import type { AffectedNode, AffectedOutcome } from './usePathwayProgressForCredential';
...
const affectedNode = (overrides: Partial<AffectedNode> = {}): AffectedNode => ({
    pathwayId: 'p1',
...
const affectedOutcome = (overrides: Partial<AffectedOutcome> = {}): AffectedOutcome => ({
    pathwayId: 'p1',
    pathwayTitle: 'AWS Cloud Practitioner',
    outcomeId: 'o1',
...
describe('humanizeNodeTitle', () => {
    it('strips short imperative-verb prefixes', () => {
        expect(humanizeNodeTitle('Earn: AWS Cloud Practitioner credential')).toBe(
            'AWS Cloud Practitioner credential'
        );

        expect(humanizeNodeTitle('Watch: AWS Cloud Essentials')).toBe('AWS Cloud Essentials');

        expect(humanizeNodeTitle('Upload: your completion certificate')).toBe(
            'your completion certificate'
        );
    });

    it('strips multi-word prefixes', () => {
        expect(humanizeNodeTitle('AI Tutor: IAM deep dive')).toBe('IAM deep dive');

        expect(humanizeNodeTitle('Deep dive: gap close')).toBe('gap close');
    });

    it('preserves titles without a prefix', () => {
        expect(humanizeNodeTitle('Complete the full course')).toBe('Complete the full course');

        expect(humanizeNodeTitle('IAM roles reflection')).toBe('IAM roles reflection');
    });
    ...
    it('preserves titles where the colon is too deep to be a prefix', () => {
        ...
        expect(humanizeNodeTitle('Reflect on what you learned: what was hardest?')).toBe(
            'Reflect on what you learned: what was hardest?'
        );
    });
    ...
    it('preserves titles where the pre-colon segment is not alphabetical', () => {
        ...
        expect(humanizeNodeTitle('http://example.com reading')).toBe('http://example.com reading');

        expect(humanizeNodeTitle('10:30 standup')).toBe('10:30 standup');

        expect(humanizeNodeTitle('Q&A: weekly review')).toBe('Q&A: weekly review');
    });
    ...
    it('handles leading/trailing whitespace after strip', () => {
        ...
        expect(humanizeNodeTitle('Watch:   AWS basics   ')).toBe('AWS basics');
    });
});
...
describe('computeProgressTier', () => {
    ...
    it('returns `cross-pathway` when the event advanced nodes on 2+ pathways', () => {
        const p1 = makePathway('p1', [makeNode('n1', 'completed'), makeNode('n2', 'not-started')]);

        const p2 = makePathway('p2', [makeNode('n1', 'completed'), makeNode('n2', 'not-started')]);
...
describe('resolveCtaCopy', () => {
    it('terminal: celebrates the whole-pathway completion', () => {
        const copy = resolveCtaCopy({
            tier: 'terminal',
            affectedNodes: [affectedNode({ pathwayTitle: 'AWS Cloud Practitioner' })],
            affectedOutcomes: [affectedOutcome()],
        });
    ...
    it('terminal: falls back to outcome pathway title when no nodes were affected', () => {
        const copy = resolveCtaCopy({
            tier: 'terminal',
            affectedNodes: [],
            affectedOutcomes: [affectedOutcome({ pathwayTitle: 'Security+' })],
        });
    ...
    it('cross-pathway: pluralises and lists pathway titles', () => {
        ...
        expect(copy.subhead).toBe('Across AWS, Security+, and Azure Fundamentals.');
    ...
    it('minor: focuses on the outcome pathway', () => {
        const copy = resolveCtaCopy({
            tier: 'minor',
            affectedNodes: [],
            affectedOutcomes: [affectedOutcome({ pathwayTitle: 'AWS Cloud Practitioner' })],
        });

        expect(copy.headline).toBe('Added to your progress.');
        expect(copy.subhead).toBe('This credential counts toward AWS Cloud Practitioner.');
    });
