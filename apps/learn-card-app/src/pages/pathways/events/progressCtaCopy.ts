...
export type ProgressTier = 'terminal' | 'cross-pathway' | 'major' | 'minor' | 'none';
...
export const computeProgressTier = (input: ComputeProgressTierInput): ProgressTier => {
...
export const resolveCtaCopy = (input: CtaCopyInput): CtaCopy => {
    ...
    switch (tier) {
        ...
        case 'cross-pathway': {
            ...
            const pathwayTitles = Array.from(new Set(affectedNodes.map(n => n.pathwayTitle)));

            const pathwayList = formatPathwayList(pathwayTitles);

            return {
                headline: `This credential advanced ${nodeCount} step${
                    nodeCount === 1 ? '' : 's'
                }.`,
