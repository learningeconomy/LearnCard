...
const makePathway = (nodes: PathwayNode[], edges: Edge[], destinationNodeId?: string): Pathway => ({
    id: 'p1',
...
describe('detectCollections', () => {
    it('detects a clean fan-in group of 4 identical siblings', () => {
        ...
        const siblings = Array.from({ length: 4 }, (_, i) => makeNode({ title: `Badge ${i + 1}` }));
    ...
    it('does not detect when there are fewer than MIN_COLLECTION_SIZE siblings', () => {
        ...
        const siblings = Array.from({ length: MIN_COLLECTION_SIZE - 1 }, () => makeNode());
    ...
    it('groups members that share the SAME incoming prereq (the new behaviour)', () => {
        ...
        const sibs = Array.from({ length: 5 }, (_, i) => makeNode({ title: `Badge ${i + 1}` }));
        const target = makeNode({ title: 'Certificate' });
        ...
        expect(new Set(groups[0].incomingEdgeIds)).toEqual(new Set(incomingEdges.map(e => e.id)));
    ...
    it('groups members that share MULTIPLE identical incoming prereqs', () => {
        ...
        const edges = [
            ...sibs.flatMap(s => [edge(upstreamA.id, s.id), edge(upstreamB.id, s.id)]),
            ...sibs.map(s => edge(s.id, target.id)),
        ];

        const pathway = makePathway([upstreamA, upstreamB, ...sibs, target], edges);
    ...
    it('refuses to group if a shared prereq is also a member', () => {
        ...
        const pathway = makePathway([memberA, memberB, memberC, memberD, target], edges);
    ...
    it('detects two independent groups in the same pathway', () => {
        ...
        const badges = Array.from({ length: 4 }, (_, i) => makeNode({ title: `Badge ${i}` }));
        const courses = Array.from({ length: 5 }, (_, i) => makeNode({ title: `Course ${i}` }));
        ...
        const pathway = makePathway([...badges, ...courses, certificate, degree], edges);
