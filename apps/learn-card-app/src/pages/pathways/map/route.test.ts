...
describe('computeSuggestedRoute', () => {
    ...
    it('walks a linear chain from focus to destination', () => {
        ...
        const p = makePathway([a, b, c], [edge(a.id, b.id), edge(b.id, c.id)], c.id);
    ...
    it('includes every uncompleted sibling in a fan-in to the destination', () => {
        ...
        const p = makePathway(
            [s1, s2, s3, dest],
            [edge(s1.id, dest.id), edge(s2.id, dest.id), edge(s3.id, dest.id)],
            dest.id
        );
    ...
    it('elides completed siblings from nodeIds but keeps their edges in the ribbon', () => {
        ...
        const p = makePathway(
            [s1Done, s2, s3, dest],
            [edge(s1Done.id, dest.id), edge(s2.id, dest.id), edge(s3.id, dest.id)],
            dest.id
        );
    ...
    it('keeps the focus at position 0 even when siblings come before it in pathway order', () => {
        ...
        const p = makePathway(
            [s1, s2, focus, dest],
            [edge(s1.id, dest.id), edge(s2.id, dest.id), edge(focus.id, dest.id)],
            dest.id
        );
    ...
    it('returns all three hops of a linear chain when siblings exist off the chain', () => {
        ...
        const p = makePathway(
            [a, b, direct, dest],
            [edge(a.id, b.id), edge(b.id, dest.id), edge(direct.id, dest.id)],
            dest.id
        );
