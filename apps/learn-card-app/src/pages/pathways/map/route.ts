 ...
 */
export const computeSuggestedRoute = (pathway: Pathway, focusId: string): SuggestedRoute | null => {
    ...
    if (focusId === destinationId) {
        ...
        const eta = node.progress.status === 'completed' ? 0 : nodeEffortMinutes(node);
    ...
    while (ready.length > 0) {
        ...
        for (const dep of dependents.get(at) ?? []) {
            ...
            if (next === 0) {
                ...
                const insertIdx = ready.findIndex(id => compareByNodeOrder(dep, id) < 0);
...
export const buildRouteIndex = (route: SuggestedRoute, pathway: Pathway): RouteIndex => {
...
export const buildRouteFromChosen = (pathway: Pathway): SuggestedRoute | null => {
    ...
    const destinationId = pathway.destinationNodeId ?? validIds[validIds.length - 1]!;
 ...
 */
export const getPathwayRoute = (pathway: Pathway, focusId: string): SuggestedRoute | null => {
