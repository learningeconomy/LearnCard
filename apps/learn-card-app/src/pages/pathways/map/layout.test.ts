...
import { X_SPACING, Y_SPACING, layoutPathway, layoutPathwayNavigate } from './layout';
...
describe('layoutPathway (bottom-up)', () => {
    ...
    it('produces a position for every node even if the graph has cycles or orphans', () => {
        ...
        // laid out somewhere.
        const p = pathway([node('a'), node('b')], [edge('1', 'a', 'b'), edge('2', 'b', 'a')]);
...
describe('layoutPathwayNavigate', () => {
    ...
    it('omits off-route nodes that are siblings of a route step', () => {
        ...
        const p = pathway(
            ...
            [edge('1', 'b', 'a'), edge('2', 'd', 'a'), edge('3', 'e', 'a'), edge('4', 'a', 'c')]
        );
    ...
    it('omits orphans (unreachable nodes) from the Navigate layout', () => {
        ...
        const p = pathway([node('a'), node('c'), node('z')], [edge('1', 'a', 'c')]);
    ...
    it('defensively drops stale route ids that do not exist in the pathway', () => {
        ...
        const positions = byId(layoutPathwayNavigate(p, ['a', 'ghost', 'b']));
    ...
    it('falls back to layoutPathway when the only valid route ids leave < 2', () => {
        ...
        const positions = byId(layoutPathwayNavigate(p, ['a', 'ghost-1', 'ghost-2']));
    ...
    describe('spine compression via collapsedMemberToGroup', () => {
        ...
        it('only compresses members of the same group, not unrelated neighbors', () => {
            ...
            const p = pathway(
                [node('a'), node('m1'), node('m2'), node('x'), node('m3'), node('b')],
                []
            );
            ...
            const positions = byId(
                layoutPathwayNavigate(p, ['a', 'm1', 'm2', 'x', 'm3', 'b'], collapsedMemberToGroup)
            );
        ...
        it('treats two separated runs of the same group as separate slots', () => {
            ...
            const p = pathway([node('a'), node('g1'), node('x'), node('g2'), node('b')], []);
            ...
            const positions = byId(
                layoutPathwayNavigate(p, ['a', 'g1', 'x', 'g2', 'b'], collapsedMemberToGroup)
            );
        ...
        it('reduces to the pre-compression behavior when the map is undefined', () => {
            ...
            const withMap = byId(layoutPathwayNavigate(p, ['a', 'b', 'c'], new Map()));
            const withoutMap = byId(layoutPathwayNavigate(p, ['a', 'b', 'c']));
