...
_A critical read of "Architecting Dynamic Learning Pathways" with recommendations for what to build, what to cut, and what to rethink._
...
-   _Initiation set_: preconditions to start
-   _Option policy_: what the learner does during the stage (frequency, artifacts, practices)
-   _Termination condition_: what evidence ends the stage
...
-   **Pathway decay.** The document mentions deprecation of bad pathways via community feedback, but not of individual _user pathways_ that go stale when the user's goals shift. How does the system gracefully sunset an abandoned path?
