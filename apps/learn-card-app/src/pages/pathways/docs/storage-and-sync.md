...
> strictly on _where state lives_ and _how it moves_.
...
references to shared entities**. That shape is _native_ to the split
we already have:
...
The cleartext fields are ones the _server itself_ needs to act on
...
#### Collection: `pathway-version` _(optional, enables time-travel)_
...
A lightweight `Pathway` node exists here _only_ to participate in the
...
-   **Authoritative revision** lives in learn-cloud (Mongo doc).
    Neo4j's `revision` is _advisory_ — it's a freshness hint for graph
...
type PathwayShareScope = {
    ...
    expiresAt?: string;
    role: 'guidance-counselor' | 'guardian' | 'teacher' | 'mentor' | 'peer' | 'institution';
};
```
...
brain-service: "give me learner X's active pathways _where
counselor.did has read.structure_ for each". The consent check is
...
_signal_ the node author should review.
...
store with Mongo for docs. That sketch was _before_ noticing that
