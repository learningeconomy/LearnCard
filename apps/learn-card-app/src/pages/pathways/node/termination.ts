...
import { computePathwayProgress, type PathwayMap } from '../core/composition';
...
const pluralize = (n: number, single: string, plural: string = `${single}s`): string =>
    `${n} ${n === 1 ? single : plural}`;
 ...
 */
const countMatchingArtifacts = (node: PathwayNode, artifactType: string): number =>
    node.progress.artifacts.filter(a => a.artifactType === artifactType).length;
    ...
    switch (termination.kind) {
        ...
        case 'endorsement': {
            ...
            return {
                ...
                unmetHint: done ? '' : `Waiting on ${pluralize(remaining, 'vouch', 'vouches')}.`,
            };
        }
        ...
        case 'pathway-completed': {
            ...
            if (!nested) {
                ...
                return {
                    ...
                    unmetHint: "We couldn't find the referenced pathway. Open Build to relink it.",
                };
            }
