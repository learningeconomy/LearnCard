...
const SubmissionForm: React.FC = () => {
    ...
    const hasUnsavedChanges = useCallback(() => {
        const keys: (keyof AppStoreListingCreate)[] = [
            'display_name',
            'tagline',
            'full_description',
            'icon_url',
    ...
    const saveDraft = async (): Promise<boolean> => {
        ...
        try {
            const listingData = {
                ...
                full_description: formData.full_description || 'Draft',
                icon_url:
                    formData.icon_url || 'https://placehold.co/128x128/e2e8f0/64748b?text=Draft',
                launch_type: formData.launch_type || 'DIRECT_LINK',
                launch_config_json: formData.launch_config_json || '{}',
                category: formData.category,
                is_plugin: formData.is_plugin,
                promo_video_url: formData.promo_video_url,
                privacy_policy_url: formData.privacy_policy_url,
                terms_url: formData.terms_url,
                ios_app_store_id: formData.ios_app_store_id,
    ...
    if (isEditMode && !listingFromState && isLoadingListing)
        return (
            <IonPage>
