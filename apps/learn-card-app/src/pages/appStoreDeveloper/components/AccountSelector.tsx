...
export interface AccountSelectorProps {
    ...
    onContinue?: (account: AccountProfile) => void;
    compact?: boolean;
}
...
export const AccountSelector: React.FC<AccountSelectorProps> = ({
    ...
    useEffect(() => {
        if (externalSelectedAccount) {
            setSelectedProfile(externalSelectedAccount);
        }
    ...
    const handleSelectProfile = (account: AccountProfile) => {
    ...
    const handleSelectExistingProfile = async (profile: LCNProfile, manager: LCNProfile) => {
        ...
        onAccountSwitch?.(accountProfile);
    };
    ...
    const handleUseParentAccount = async () => {
        ...
        handleSelectProfile(parentProfile);
    ...
    const createManagedServiceProfile = async () => {
        ...
        try {
            ...
            const newOrgProfile: AccountProfile = {
            ...
            {showInfoBanner && mode === 'select' && (
                <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                    ...
                    <div className="text-sm text-blue-800">
                        ...
                        <p>
                            Choose which account to use. This determines the DID, API keys, and
                            branding.
                        </p>
                    </div>
                </div>
            ...
            {mode === 'select' ? (
                <>
                    ...
                    {showContinueButton && (
                        <div className="pt-3 border-t border-gray-100">
                            <button
                                onClick={() => canProceed && onContinue?.(selectedProfile!)}
                                disabled={!canProceed}
                                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-cyan-500 text-white rounded-xl font-medium hover:bg-cyan-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
                            >
                                {continueButtonText}
                            </button>
                        </div>
                ...
                <div className="space-y-5">
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-violet-500 text-white rounded-xl font-medium hover:bg-violet-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
                    >
                        {isCreating ? (
                            <>
                                <Loader2 className="w-4 h-4 animate-spin" />
