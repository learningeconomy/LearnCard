...
const log = getLogger('project-setup-step');
...
export const ProjectSetupStep: React.FC<ProjectSetupStepProps> = ({
    ...
    const handleSelectExistingIntegration = (integration: {
        id: string;
        name: string;
        createdAt?: string;
    }) => {
        ...
        try {
            ...
            await wallet.invoke.addAuthGrant({
                ...
                description: `API key for ${
                    createdProject?.name || projectName || 'partner'
                } integration`,
                scope: '*:*',
            });
            ...
            log.error('Failed to create grant:', err);
        } finally {
            ...
            <div className="space-y-4">
                ...
                {projectSelected && !showCreateNew && (
                    <div className="flex items-center gap-3 p-3 bg-emerald-50 border border-emerald-200 rounded-xl ml-11">
                        ...
                        >
                            {m['developerPortal.onboarding.projectSetup.change']()}
                        </button>
                    </div>
                )}
                                    ...
                                    <div className="space-y-2">
                                        {existingIntegrations.map(integration => (
                                            <button
                                                key={integration.id}
                                                onClick={() =>
                                                    handleSelectExistingIntegration(integration)
                                                }
            ...
            <div className="flex gap-3 pt-4 border-t border-gray-100">
                {onBack && (
                    ...
                    >
                        ...
                        {m['developerPortal.onboarding.projectSetup.back']()}
                    </button>
                )}

                <button
                    onClick={() =>
                        canProceed && onComplete({ ...createdProject!, apiKey: apiToken })
                    }
                    disabled={!canProceed}
