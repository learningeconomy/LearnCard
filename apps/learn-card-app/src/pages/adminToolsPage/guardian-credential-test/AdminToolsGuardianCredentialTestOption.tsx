...
const AdminToolsGuardianCredentialTestOption: FC<{ option: AdminToolOption }> = ({ option }) => {
    ...
    const removeRelationship = useMutation<boolean, Error, { profileId: string }>({
        ...
        onError: err => {
            presentToast(err.message || 'Failed to remove relationship', {
    ...
    const handleSend = async () => {
        ...
        if (recipientEmail.trim().toLowerCase() === guardianEmail.trim().toLowerCase()) {
            presentToast('Guardian email must differ from recipient', {
                type: ToastTypeEnum.Error,
            });
            return;
        }
                ...
                <div className="p-[20px] flex flex-col gap-[20px]">
                    ...
                    <div>
                        ...
                        <p className="text-[12px] text-grayscale-400 font-notoSans mt-[4px]">
                            Must differ from recipient. Guardian will receive an approval link and
                            OTP challenge.
                        </p>
                    </div>
                    ...
                    {status === 'error' && result?.error && (
                        <div className="bg-red-50 border border-red-200 rounded-[12px] p-[16px] flex flex-col gap-[6px]">
                            <p className="text-[14px] font-[600] text-red-800 font-notoSans">
                                Error
                            </p>
                    ...
                    <div>
                        ...
                        {!guardiansLoading && guardiansError && (
                            <p className="text-[13px] text-red-500 font-notoSans">
                                Error:{' '}
                                {guardiansError instanceof Error
