...
import ConsentFlowAiSessionsWriteDetailsModal from './ConsentFlowAiSessionsWriteDetailsModal';

type ConsentFlowAiSessionsWriteItemProps = {
    aiSessionCategories: {
        [key: string]: {
            required: boolean;
...
const ConsentFlowAiSessionsWriteItem: React.FC<ConsentFlowAiSessionsWriteItemProps> = ({
    aiSessionCategories,
    contractDetails,
    app,
}) => {
    ...
    const numEnabled = Object.values(aiSessionCategories).filter(category => category.term).length;
    ...
    return (
        <li className="border-b border-solid border-grayscale-200 last:border-none text-black hover:bg-grayscale-100 transition-colors">
            <button
                ...
                onClick={() => {
                    newModal(
                        <ConsentFlowAiSessionsWriteDetailsModal
...
export default ConsentFlowAiSessionsWriteItem;
