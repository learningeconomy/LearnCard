...
import { DEFAULT_HTTPS_DOMAINS, isTenantHttpsUrl, parseClaimInput } from './parseClaimInput';
...
describe('parseClaimInput', () => {
    ...
    describe('VC-API custom schemes', () => {
        ...
        it('preserves the pathname (no /request prefix) when the URL already has one', () => {
            const result = parseClaimInput('dccrequest://issuer.example.com/abc?challenge=xyz');
            expect(result.kind).toBe('vc-api-custom-scheme');
            if (result.kind === 'vc-api-custom-scheme') {
    ...
    describe('default tenant HTTPS domains', () => {
        ...
        it('recognizes Netlify preview hosts by default', () => {
            expect(parseClaimInput('https://learncardapp.netlify.app/some/path').kind).toBe(
                'lcw-https'
            );
            expect(parseClaimInput('https://learncardapp.netlify.com/some/path').kind).toBe(
                'lcw-https'
            );
        });
    });
...
describe('isTenantHttpsUrl', () => {
    ...
    it('respects custom tenant config', () => {
        expect(isTenantHttpsUrl('https://vetpass.app/x', { httpsDomains: ['vetpass.app'] })).toBe(
            true
        );
        expect(isTenantHttpsUrl('https://learncard.app/x', { httpsDomains: ['vetpass.app'] })).toBe(
            false
        );
    });
