...
export const globalSetup = async (config: FullConfig) => {
    ...
    log.info(`[GlobalSetup] Using browser: ${projectName || 'chromium'}`);
    ...
    log.info('[GlobalSetup] Email input visible, filling email');
    ...
    if (userState.state.currentUser) {
        ...
        log.info('[GlobalSetup] Saved demoState.json with currentUser:', {
            uid: userState.state.currentUser.uid,
            email: userState.state.currentUser.email,
            name: userState.state.currentUser.name,
        });
    } else {
        log.error('[GlobalSetup] WARNING: currentUser is null, storage state may be invalid!');
    }
    ...
    if (userState2.state.currentUser) {
        ...
        log.info('[GlobalSetup] Saved test2State.json with currentUser:', {
            uid: userState2.state.currentUser.uid,
            email: userState2.state.currentUser.email,
            name: userState2.state.currentUser.name,
        });
    } else {
        log.error('[GlobalSetup] WARNING: currentUser2 is null, storage state may be invalid!');
    }

    log.info('[GlobalSetup] Setup complete, closing browser');
