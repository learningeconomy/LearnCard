...
| **Current (verified Feb 18, 2026)** | **8,945 KB** | **2,324 KB** | Confirmed via fresh `bun run build` |
...
After Phase 1, the remaining 11.8 MB main chunk was profiled with `ANALYZE=true bun run build`. Several large, eagerly-loaded dependencies were identified and moved out of the critical path.
...
Fresh build run: `NODE_OPTIONS="--max-old-space-size=16608" bun run build`
