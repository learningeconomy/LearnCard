...
export default defineConfig(async () => ({
    ...
    define: {
        ...
        SENTRY_ENV: process.env.SENTRY_ENV ? `"${process.env.SENTRY_ENV}"` : '"scouts-development"',
        ...
        WEB3AUTH_MAINNET_CLIENT_ID:
            process.env.WEB3AUTH_MAINNET_CLIENT_ID && `"${process.env.WEB3AUTH_MAINNET_CLIENT_ID}"`,
        WEB3AUTH_TESTNET_CLIENT_ID:
            process.env.WEB3AUTH_TESTNET_CLIENT_ID && `"${process.env.WEB3AUTH_TESTNET_CLIENT_ID}"`,
    },
    resolve: {
        alias: [
            ...
            {
                find: /^learn-card-base\/(.*)$/,
                replacement: path.resolve(__dirname, '../../packages/learn-card-base/src') + '/$1',
            },
            {
