...
const CreateFrameworkModal: React.FC<CreateFrameworkModalProps> = ({ isEdit, frameworkInfo }) => {
    ...
    useEffect(() => {
        if (existingNetworks) {
            // TODO doesn't handle pagination
            setSelectedNetworks(existingNetworks.records.map(network => network.uri));
        }
    }, [existingNetworks]);
    ...
    const createFrameworkMutation = useMutation({
        ...
        onError: error => {
            ...
            alert(m['skillFrameworks.toasts.createFail']());
        },
    });

    const handleSelectNetwork = () => {
        newModal(
            <SkillsFrameworkNetworkSelector
                selectedNetworks={selectedNetworks}
                onSelectNetworks={networks => setSelectedNetworks(networks)}
            />,
                ...
                <div className="flex flex-col gap-[20px] bg-white p-[20px] rounded-[20px] shadow-bottom-0-4">
                    {!isEdit && (
                        <p className="font-poppins text-[14px] text-grayscale-90 w-full text-center">
                            {m['skillFrameworks.startNewDesc']()}
                        </p>
                    )}
                    ...
                    {selectedNetworks.length > 0 && (
                        <div className="flex flex-col">
                            {selectedNetworks.map(network => (
                                <SimpleAttachedNetworkDisplay key={network} networkUri={network} />
                            ))}
                        </div>
                    )}
                </div>
