...
| Data Sources | `INTEGRATION` subset  | Integrations whose signed manifest provides the `insight-source` capability (ADR-008 D6, ADR-013 Q4) — non-subject reference data _in_, NOT a separate listing kind. Record classes mark the SUBJECT-DATA lane and render as pills on the Integrations page instead. Tabs are MUTUALLY EXCLUSIVE: insight-source-only feeds are hidden from Integrations; hybrids (subject-data + insight) stay on Integrations (strictest lane wins, ADR-013) |
