...
export function CreateEcosystemForm({
    ...
    onCreated,
    onCancel,
}: CreateEcosystemFormProps) {
    const [name, setName] = useState('');
    const [slug, setSlug] = useState('');
    const [description, setDescription] = useState('');
    const [parentId, setParentId] = useState(fixedParentId || (parentOptions[0]?.id ?? ''));
    ...
    const canSubmit = name.trim().length > 0 && isValidSlug && parentId;
