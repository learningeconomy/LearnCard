 ...
 * out-of-band channel.
 */
const injectTxCodeIntoOffer = async (offerUri: string, pin: string): Promise<string> => {
    ...
    if (!preAuthGrant) {
        throw new Error(
            `Offer has no pre-authorized_code grant — cannot simulate PIN gating (grants: ${
                Object.keys(grants).join(', ') || 'none'
            })`
        );
    }
 ...
 */
export const mintWaltidSdJwtOffer = async (opts: MintSdJwtOfferOptions): Promise<string> => {
    const {
        issuerBaseUrl,
        issuerKey,
 ...
 */
export const mintWaltidBatchOffer = async (opts: MintBatchOfferOptions): Promise<string> => {
...
export const overridePresentationDefinition = (
    ...
    if (queryStart === -1) {
        throw new Error(`Authorization request URI has no query string: ${authRequestUri}`);
    }
