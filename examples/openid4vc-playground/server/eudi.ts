...
export const createEudiVerifySession = async (
    ...
    const nonce =
        opts.nonce ??
        Array.from(crypto.getRandomValues(new Uint8Array(16)), (b: number) =>
    ...
    if (!res.ok) {
        ...
        throw new Error(`EUDI /ui/presentations failed ${res.status}: ${text}`);
    }
    ...
    if (jarMode === 'by_value') {
        if (!json.request) {
            throw new Error('EUDI returned no `request` field for jar_mode=by_value');
        }
        params.set('request', json.request);
    } else {
        if (!json.request_uri) {
            throw new Error('EUDI returned no `request_uri` for jar_mode=by_reference');
        }
...
export const getEudiVerifyStatus = async (
    ...
    if (!res.ok) {
        const text = await res.text().catch(() => '');
        throw new Error(`EUDI /ui/presentations/${transactionId} failed ${res.status}: ${text}`);
    }
