...
function inferSpec(cred: Record<string, unknown>): CredentialSpec | undefined {
    ...
    // CLR v2
    if (types.some(t => t === 'ClrCredential') || contexts.some(c => c.includes('clr'))) {
        return 'clr-v2';
    }
    ...
    if (
        types.some(t =>
            ['OpenBadgeCredential', 'AchievementCredential', 'BoostCredential'].includes(t)
        ) ||
...
function inferProfile(
    ...
    if (subject) {
        const subjectTypes = asArray(subject.type).filter(
