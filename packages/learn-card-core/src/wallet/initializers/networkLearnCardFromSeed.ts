...
import { getLearnCardNetworkPlugin } from '@wallet/plugins/learn-card-network';
...
export const networkLearnCardFromSeed = async ({
    ...
    debug,
}: Partial<NetworkLearnCardFromSeed['args']> & { seed: string; network: string }): Promise<
    NetworkLearnCardFromSeed['returnValue']
> => {
    ...
    const learnCardLc = await chapiLc.addPlugin(getLearnCardPlugin(chapiLc));

    return learnCardLc.addPlugin(await getLearnCardNetworkPlugin(learnCardLc, network));
};
