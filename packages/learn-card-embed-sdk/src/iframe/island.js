...
(function () {
    ...
    if (!preact) {
        throw new Error('LearnCard Embed: Preact not available in island');
    }
    var h = preact.h,
                ...
                return h('input', {
                    ...
                    onKeyDown: function (ev) {
                        if (ev.key === 'Backspace' && !ev.target.value && i > 0) {
                            var prev = ev.target.parentElement.querySelectorAll('input')[i - 1];
                            if (prev) prev.focus();
                        }
                    },
                });
    ...
    function EmailView(props) {
        return h('div', { id: 'view-email' }, [
            ...
            h(
                'div',
                { class: 'subtitle' },
                'Enter your email to receive a one-time code to add this credential to your LearnCard.'
            ),
            h('div', { class: 'field' }, [
            ...
            h('div', { class: 'center' }, [
                h(
                    'button',
                    { id: 'send', class: 'btn btn-primary', onClick: props.onSend },
                    'Send Code'
                ),
            ]),
    ...
    function OtpView(props) {
        return h('div', { id: 'view-otp' }, [
            ...
            h('div', { class: 'subtitle' }, [
                'We sent a code to ',
                h('span', { id: 'email-disp' }, props.email),
        ...
        return h('div', { id: 'view-success' }, [
            h('div', { class: 'success' }, [
                ...
                h(
                    'div',
                    { class: 'subtitle' },
                    'Your credential has been added to your LearnCard.'
                ),
                props.showConsent
                    ? h('label', { class: 'check', id: 'consent-wrap' }, [
                          h('input', {
                              id: 'consent',
                              type: 'checkbox',
                              checked: props.consent,
                              onChange: function (e) {
                                  props.onConsent(e.target.checked);
                              },
                          }),
                          h(
                              'span',
                              ...
                              'Allow ' +
                                  props.partnerLabel +
                                  ' to automatically add future credentials to my LearnCard.'
                          ),
                      ])
                    : null,
                h('div', { class: 'center' }, [
                    h(
                        'button',
                        { id: 'viewWallet', class: 'btn btn-primary', onClick: props.onViewWallet },
                        'View My LearnCard'
                    ),
                ]),
            ]),
    ...
    function Root(props) {
        ...
        var App = (function (_super) {
            function App() {
                ...
                this.state = { view: 'email', email: '', code: '', error: '', consent: false };
            }
            App.prototype.onSend = function () {
                ...
                var self = this;
                setTimeout(function () {
                    self.setState({ view: 'otp' });
                }, 300);
            };
            App.prototype.onVerify = function () {
                var code = this.state.code || '';
                if (code.length !== 6) {
                    this.setState({ error: 'Enter the 6-digit code.' });
                    return;
                }
                var self = this;
                this.setState({ error: '' });
                setTimeout(function () {
                    self.setState({ view: 'success' });
                }, 300);
            };
            App.prototype.onViewWallet = function () {
                var payload = {
                    ...
                    payload: {
                        ...
                        consentGiven: !!this.state.consent,
                    },
                };
            ...
            App.prototype.render = function () {
                if (this.state.view === 'email')
                    return h('div', { class: 'wrap' }, [
                        h(EmailView, {
                            ...
                            onEmail: v => this.setState({ email: v }),
                ...
                return h('div', { class: 'wrap' }, [
                    h(SuccessView, {
                        ...
                        showConsent: !!config.showConsent,
                        consent: this.state.consent,
                        onConsent: v => this.setState({ consent: v }),
                        onViewWallet: this.onViewWallet.bind(this),
                    }),
                ]);
            };
            return App;
