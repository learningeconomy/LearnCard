...
export const AppManifestValidator = z
    .object({
        ...
        appUrl: z.string().max(2048),
        suggestedName: z.string().max(200).optional(),
        suggestedIconUrl: z.string().max(2048).optional(),
        permissions: z.array(z.string()).max(50).default([]),
        templates: z.array(CapturedTemplateRecordValidator).max(50).default([]),
        consentRequests: z.array(CapturedConsentRecordValidator).max(50).default([]),
        featuresLaunched: z.array(z.string()).max(100).default([]),
        counterKeys: z.array(z.string()).max(100).default([]),
