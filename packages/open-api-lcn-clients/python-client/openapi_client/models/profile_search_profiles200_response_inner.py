...
class ProfileSearchProfiles200ResponseInner(BaseModel):
    ...
    connected_at: Optional[datetime] = Field(default=None, description="When the viewer and this profile became connected.", alias="connectedAt")
    ...
    locale: Optional[StrictStr] = Field(default=None, description="BCP-47 language tag (e.g. 'es', 'fr', 'ar') — the user's preferred language for server-sent notifications and emails.")
    approved: Optional[StrictBool] = Field(default=None, description="Approval status for the profile.")
    __properties: ClassVar[List[str]] = ["connectionStatus", "profileId", "displayName", "shortBio", "image", "heroImage", "type", "isServiceProfile", "display", "bio", "websiteLink", "role", "highlightedCredentials", "did", "email", "connectedAt", "isPrivate", "profileVisibility", "showEmail", "allowConnectionRequests", "notificationsWebhook", "dob", "country", "locale", "approved"]
    ...
    def connected_at_validate_regular_expression(cls, value):
    ...
    @field_validator('notifications_webhook', mode="before")
    def notifications_webhook_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if isinstance(value, str) and not re.match(r"^http.*", value):
