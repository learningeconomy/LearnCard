...
class ClaimHookCreateClaimHookRequestHook(BaseModel):
    ...
    """
    # data type: ClaimHookCreateClaimHookRequestHookOneOf
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        error_messages = []
        match = 0
        # validate data type: ClaimHookCreateClaimHookRequestHookOneOf
        if not isinstance(v, ClaimHookCreateClaimHookRequestHookOneOf):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ClaimHookCreateClaimHookRequestHookOneOf`")
        else:
            match += 1
        # validate data type: ClaimHookCreateClaimHookRequestHookOneOf1
        if not isinstance(v, ClaimHookCreateClaimHookRequestHookOneOf1):
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ClaimHookCreateClaimHookRequestHookOneOf
        try:
            instance.actual_instance = ClaimHookCreateClaimHookRequestHookOneOf.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ClaimHookCreateClaimHookRequestHookOneOf1
        try:
            instance.actual_instance = ClaimHookCreateClaimHookRequestHookOneOf1.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
