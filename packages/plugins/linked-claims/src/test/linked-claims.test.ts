/* eslint-disable @typescript-eslint/no-explicit-any -- legacy plugin mocks intentionally model flexible LearnCard surfaces. */
...
describe('LinkedClaims Plugin', () => {
    test('endorseCredential: minimal (comment only) with original id', async () => {
        ...
        expect(types).toEqual(
            expect.arrayContaining(['VerifiableCredential', 'EndorsementCredential'])
        );
    ...
    test('endorseCredential: rejects a subject DID as a credential target', async () => {
        ...
        await expect(
            plugin.methods.endorseCredential(
                lc,
                original,
                { name: 'Kudos', description: 'Peer endorsement' },
                {}
            )
        ).rejects.toThrow('Original credential must have an id');
    });
    ...
    test('verifyEndorsement: subject type missing EndorsementSubject -> error', async () => {
        ...
        expect(res.errors.join(' ')).toMatch(
            /credentialSubject\.type must include EndorsementSubject/
        );
    });
