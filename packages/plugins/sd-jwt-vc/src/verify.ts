...
const resolveIssuerJwk = async (
    ...
    if (!isAuthorizedForAssertion(doc, method.id, issuerDid)) {
        throw new SdJwtVcError(
            ...
            `Verification method "${
                method.id ?? '(none)'
            }" is not in the assertionMethod set of issuer "${issuerDid}", so it cannot sign credentials per W3C DID Core §5.3.2`
        );
    }
...
const ensureNotExpired = (expiresAt: Date | undefined, now: Date, errors: string[]): void => {
