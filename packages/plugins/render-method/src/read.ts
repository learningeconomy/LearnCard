...
export const getRenderMethods = (vc: AnyCredential): RenderMethod[] => {
    const effective = (unwrapBoostCredential(vc) ?? vc) as Record<string, unknown> | undefined;
 ...
 */
export const getSvgMustacheRenderMethod = (vc: AnyCredential): TemplateRenderMethod | null =>
    findTemplateRenderMethod(vc, 'svg-mustache');
...
const buildRenderDataFromPointers = (vc: AnyCredential, pointers: string[]): JsonRecord => {
 ...
 * unwraps.
 */
export const buildRenderData = (vc: AnyCredential, renderProperty?: string[]): RenderMethodData => {
