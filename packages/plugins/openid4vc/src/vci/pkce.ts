 ...
 */
export const generatePkcePair = async (options: GeneratePkceOptions = {}): Promise<PkcePair> => {
...
const getCrypto = (): Crypto => {
    ...
    if (!c) {
        throw new PkceError('invalid_verifier', 'Web Crypto API not available in this runtime');
    }
    return c;
};
...
const getSubtle = (): SubtleCrypto => {
    ...
    if (!subtle) {
        throw new PkceError('invalid_verifier', 'crypto.subtle not available in this runtime');
    }
    return subtle;
};
