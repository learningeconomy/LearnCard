import { computeS256Challenge, generatePkcePair, PkceError, verifyPkce } from './pkce';
...
describe('verifyPkce', () => {
    ...
    it('returns false when the verifier does not match the challenge', async () => {
        const a = await generatePkcePair();
        const b = await generatePkcePair();
        expect(await verifyPkce({ verifier: a.verifier, challenge: b.challenge })).toBe(false);
    });
