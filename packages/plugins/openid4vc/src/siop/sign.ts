...
export type SiopSignErrorCode = 'invalid_input' | 'missing_signer' | 'id_token_sign_failed';

export class SiopSignError extends Error {
    readonly code: SiopSignErrorCode;

    constructor(code: SiopSignErrorCode, message: string, options?: { cause?: unknown }) {
        super(message);
...
const validateInput = (options: SignIdTokenOptions, helpers: SignIdTokenHelpers): void => {
    if (!helpers.signer) {
        throw new SiopSignError('missing_signer', 'signIdToken requires helpers.signer');
    }

    if (typeof options.holder !== 'string' || options.holder.length === 0) {
        throw new SiopSignError('invalid_input', 'signIdToken requires a non-empty `holder`');
    }

    if (typeof options.audience !== 'string' || options.audience.length === 0) {
        throw new SiopSignError('invalid_input', 'signIdToken requires a non-empty `audience`');
    }

    if (typeof options.nonce !== 'string' || options.nonce.length === 0) {
        throw new SiopSignError('invalid_input', 'signIdToken requires a non-empty `nonce`');
    }
};
