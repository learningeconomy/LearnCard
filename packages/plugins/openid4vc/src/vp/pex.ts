...
export const queryJsonPath = (root: unknown, path: string): unknown[] => {
    ...
    if (/\[\?\(/.test(path)) {
        throw new Error(`JSONPath filter expressions are not supported yet (found in \"${path}\")`);
    }
    ...
    while (i < path.length) {
        ...
        if (c === '.') {
            ...
            if (inner === '*') {
                ...
                throw new Error(
                    `Unsupported bracket expression [${inner}] in JSONPath \"${path}\"`
                );
            }
...
export const satisfiesFilter = (value: unknown, filter: Record<string, unknown>): boolean => {
    ...
    if (typeof value === 'number') {
        ...
        if (typeof filter.exclusiveMinimum === 'number' && value <= filter.exclusiveMinimum) {
            return false;
        }
        if (typeof filter.exclusiveMaximum === 'number' && value >= filter.exclusiveMaximum) {
            return false;
        }
    }
