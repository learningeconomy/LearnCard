import { queryJsonPath, satisfiesFilter, evaluateField, matchInputDescriptor } from './pex';
...
describe('queryJsonPath', () => {
    ...
    it('returns a single-element array for a simple property', () => {
        expect(queryJsonPath(sampleVc, '$.issuer')).toEqual(['did:web:issuer.example.com']);
    });
    ...
    it('supports numeric array index', () => {
        ...
        expect(queryJsonPath(sampleVc, '$.type[1]')).toEqual(['UniversityDegreeCredential']);
    });
    ...
    it('supports bracket-quoted property names', () => {
        ...
        expect(queryJsonPath(sampleVc, '$["credentialSubject"]["id"]')).toEqual(['did:jwk:eyJ...']);
    });
    ...
    it('throws on unsupported filter predicates', () => {
        expect(() => queryJsonPath(sampleVc, "$.credentialSubject[?(@.type=='X')]")).toThrow(
...
describe('satisfiesFilter', () => {
    ...
    describe('const', () => {
        ...
        it('matches deep equality on arrays and objects', () => {
            ...
            expect(satisfiesFilter({ a: 1, b: 2 }, { const: { a: 1, b: 2 } })).toBe(true);
            expect(satisfiesFilter({ a: 1, b: 2 }, { const: { a: 1, b: 3 } })).toBe(false);
        });
    });
    ...
    describe('pattern', () => {
        it('matches against a regex', () => {
            expect(satisfiesFilter('did:web:example.com', { pattern: '^did:web:' })).toBe(true);
            expect(satisfiesFilter('did:key:zABC', { pattern: '^did:web:' })).toBe(false);
        });
    ...
    describe('array keywords', () => {
        it('contains: any item must satisfy the sub-filter', () => {
            expect(
                satisfiesFilter(['VerifiableCredential', 'UniversityDegreeCredential'], {
        ...
        it('items: every element must satisfy the sub-filter', () => {
            expect(satisfiesFilter([1, 2, 3], { items: { type: 'integer' } })).toBe(true);
            expect(satisfiesFilter([1, 'two', 3], { items: { type: 'integer' } })).toBe(false);
        });
