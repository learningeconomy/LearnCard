...
describe('adaptCredentialForDcql — jwt_vc_json', () => {
    it('decodes a compact JWT-VC into the canonical W3C DCQL shape', async () => { const jwt = makeJwtVc({
    ...
    it('honors an explicit format hint over the auto-inferred one', async () => { const jwt = makeJwtVc({
        type: ['VC'],
    ...
    it('extracts type from a JWT-VC inside a proof.jwt envelope', async () => { // Some legacy issuers wrap the JWT-VC in an LDP-shaped envelope
    ...
    it('returns undefined for a non-JWS string', async () => { expect(await adaptCredentialForDcql({ credential: 'not.a.real.jwt' })).toBeUndefined(); });

    it('returns undefined when JWT payload has no `vc` claim', async () => { const header = base64url(JSON.stringify({ alg: 'EdDSA' }));
    ...
    it('returns undefined when the VC has no type array', async () => { const jwt = makeJwtVc({ credentialSubject: {} });
    expect(await adaptCredentialForDcql({ credential: jwt })).toBeUndefined(); });

    it('coerces a string `type` into a single-element array', async () => { const jwt = makeJwtVc({
...
describe('adaptCredentialForDcql — ldp_vc', () => {
    it('passes a JSON-LD VC through with the right format tag', async () => { const ldVc = {
        '@context': ['https://www.w3.org/2018/credentials/v1'],
        type: ['VerifiableCredential', 'UniversityDegree'],
    ...
    it('returns undefined for a credential with no proof', async () => { // `inferDcqlFormat` only returns ldp_vc when proof is present;
    ...
    it('returns undefined for a JSON-LD VC with no type', async () => { const out = await adaptCredentialForDcql({
        credential: {
            credentialSubject: {},
...
describe('adaptCredentialForDcql — unsupported', () => {
    it('returns undefined for null / number / undefined inputs', async () => { expect(await adaptCredentialForDcql({ credential: null })).toBeUndefined();
    ...
    it('returns undefined for sd-jwt-vc compact serialization', async () => { // SD-JWT compact serialization uses tilde-delimited segments,
    ...
    expect(await adaptCredentialForDcql({
        credential: 'eyJhbGc.eyJ2Yy.sig~disclosure1~disclosure2',
    })).toBeUndefined(); });

    it('honors a non-supported explicit format hint by dropping', async () => { // Caller explicitly says vc+sd-jwt — adapter doesn't yet
    // handle that branch and returns undefined.
    const jwt = makeJwtVc({ type: ['VC'], credentialSubject: {} });
    expect(await adaptCredentialForDcql({ credential: jwt, format: 'vc+sd-jwt' })).toBeUndefined(); });
});
...
describe('adaptCredentialsForDcql — batch', () => {
    it('drops unsupported entries silently and pairs adapted with original', async () => { const jwt = makeJwtVc({
        type: ['VerifiableCredential', 'UniversityDegree'],
        credentialSubject: {},
    });
    const ldVc = {
