...
-   Activation: `mock` option — `'auto'` (default) mocks only when **no LearnCard host is present AND the page is on a local dev host** (`localhost`, `127.0.0.1`, `[::1]`, `*.localhost`, `*.local`); `true` always mocks (use for remote previews/CI); `false` never mocks. `mockOptions` tunes UI/logging/persistence/DID/namespace.
