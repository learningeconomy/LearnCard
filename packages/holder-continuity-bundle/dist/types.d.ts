...
export type BundleContentType = 'key-recovery-phrase' | 'key-private-seed' | 'key-jwks' | 'did-document' | 'credential' | 'presentation' | 'consent-record' | 'status-cache' | 'unknown-json' | 'index-record';
...
export type ReadLearnCardBundleResult = {
    manifest: LearnCardBundleManifest;
    entries: Array<LearnCardBundleEntryMetadata & {
        content: string;
    }>;
    warnings: string[];
};
...
export type LearnCardBundleWallet = {
    ...
    invoke: {
        ...
        verifyCredential?: (credential: JsonValue, options?: Record<string, unknown>) => Promise<unknown>;
        verifyPresentation?: (presentation: JsonValue, options?: Record<string, unknown>) => Promise<unknown>;
