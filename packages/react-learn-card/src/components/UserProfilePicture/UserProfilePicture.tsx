...
export const UserProfilePicture: React.FC<{
    ...
    if (!src) {
        return (
            <div
                className={`rounded-full flex items-center justify-center font-bold uppercase select-none h-full w-full text-white text-4xl ${customContainerClass} ${baseColor}`}
            >
                {letterToDisplay}
                {children}
            </div>
