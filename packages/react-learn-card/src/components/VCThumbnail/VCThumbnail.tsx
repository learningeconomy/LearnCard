...
export const VCThumbnail: React.FC<VCThumbnailProps> = ({
    ...
    return (
        <div
            onClick={onClick}
            className={`flex flex-col items-center justify-between relative w-60 h-60 py-6 px-3 shadow-2xl rounded-3xl vc-thumbail-container ${className}`}
        >
            <div className="flex flex-col items-center justify-center z-10 text-center w-full">
                <h2 className="text-lg font-normal tracking-wide leading-snug text-center">
                    {title ?? ''}
                </h2>
                <p className="text-base font-light text-center">{createdAt ?? ''}</p>

                <div className="flex flex-row items-center justify-center mt-2 w-full">
                    <div className="flex items-center justify-center h-14 w-14 shadow-2xl rounded-full overflow-hidden bg-white">
            ...
            <div className="flex items-center justify-center w-full vc-thumbnail-footer">
                <h3 className='text-white text-sm text-right font-normal tracking-wide leading-tight flex-1'>Verified</h3>
                <div className="flex items-center justify-center my-0 mx-1 h-10 w-10 rounded-full bg-white overflow-hidden">
                    <div className="flex items-center justify-center h-5/6 w-10/12 bg-white rounded-full overflow-hidden vc-thumbnail-verification-badge-wrap">
                        <img className='h-full w-full object-cover' src={VerifiedArrow ?? ''} alt="credential icon" />
                    </div>
                </div>
                <h3 className='text-white text-sm text-left font-normal tracking-wide leading-tight flex-1'>Credential</h3>
            </div>
        </div>
    );
};
