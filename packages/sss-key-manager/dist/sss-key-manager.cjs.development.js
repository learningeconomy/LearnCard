...
__export(index_exports, {
  ...
  SSSApiClient: () => SSSApiClient,
  SSSKeyManager: () => SSSKeyManager,
  SSS_THRESHOLD: () => SSS_THRESHOLD,
  SSS_TOTAL_SHARES: () => SSS_TOTAL_SHARES,
...
async function splitPrivateKey(privateKeyHex) {
  ...
  const shares = await (0, import_shamir_secret_sharing.split)(privateKeyBytes, SSS_TOTAL_SHARES, SSS_THRESHOLD);
  return {
...
async function reconstructPrivateKey(share1Hex, share2Hex) {
  ...
  const reconstructed = await (0, import_shamir_secret_sharing.combine)([share1, share2]);
...
async function reconstructFromShares(shares) {
  ...
  const reconstructed = await (0, import_shamir_secret_sharing.combine)(shareBytes);
  return bytesToHex(reconstructed);
}
