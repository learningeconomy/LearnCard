...
describe('QR Crypto (X25519 ECDH + AES-GCM)', async () => {
    ...
    it.skipIf(!supported)(
        'generateEphemeralKeypair returns a public key string and private CryptoKey',
        async () => {
    ...
    it.skipIf(!supported)('decryption with wrong private key fails', async () => {
        ...
        await expect(decryptShareFromTransfer(encrypted, wrongKey.privateKey)).rejects.toThrow();
    });

    it.skipIf(!supported)(
        'encrypted payload is different each time (unique IV + sender key)',
        async () => {
