...
describe('SSS operations', () => {
    ...
    it('should split and reconstruct a private key', async () => {
        ...
        const reconstructed4 = await reconstructFromShares([shares.deviceShare, shares.emailShare]);
        expect(reconstructed4).toBe(privateKeyHex);

        const reconstructed5 = await reconstructFromShares([shares.authShare, shares.emailShare]);
