...
import { generateEphemeralKeypair, encryptShareForTransfer } from './qr-crypto';
...
const mockFetch = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (urlStr.endsWith('/qr-login/session') && method === 'POST') {
        ...
        const shortCode = Math.floor(Math.random() * 100_000_000)
            .toString()
            .padStart(8, '0');
...
describe('QR Login Client', async () => {
    ...
    it.skipIf(!supported)(
        'createQrLoginSession returns session info and ephemeral keypair',
        async () => {
    ...
    it.skipIf(!supported)(
        'full cross-device flow: Device B creates, Device A approves, Device B decrypts',
        async () => {
            const deviceShare =
    ...
    it.skipIf(!supported)(
        'full flow with accountHint: hint is encrypted and returned to Device B',
        async () => {
            const deviceShare =
                'abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789ab';
    ...
    it.skipIf(!supported)(
        'full flow with shareVersion: version is encrypted and returned to Device B',
        async () => {
            const deviceShare =
                'abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789ab';
