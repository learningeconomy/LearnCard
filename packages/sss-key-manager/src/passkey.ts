...
export async function deriveKeyFromPasskey(credentialId: string): Promise<CryptoKey> {
    if (!isWebAuthnSupported()) {
        throw new Error('WebAuthn is not supported in this browser');
    }
    ...
    if (!extensionResults.prf?.results?.first) {
        throw new Error(
            'PRF extension not available or failed. This passkey cannot be used for encryption.'
        );
    }

    const prfOutput = new Uint8Array(extensionResults.prf.results.first);

    const cryptoKey = await crypto.subtle.importKey('raw', prfOutput, { name: 'AES-GCM' }, false, [
        'encrypt',
        'decrypt',
    ]);
