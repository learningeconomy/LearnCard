...
const SIG_RES_MESSAGE = 'No valid public key JWKs found in DID document for did:web:example';
...
describe('runWithRecovery', () => {
    it('returns the runner result on first-attempt success', async () => {
        const result = await runWithRecovery(async () => 'ok', {
    ...
    it('rethrows the original raw error when exhausted, NOT the friendly version', async () => {
        ...
        await expect(runWithRecovery(runner, { availableSigners: ['did:key'] })).rejects.toBe(raw);
    });
    ...
    it('emits orchestrator_exhausted on terminal error', async () => {
        ...
        const exhaustedEvents = telemetry.events.filter(e => e.type === 'orchestrator_exhausted');
    ...
    it('invokes onAttempt with the previous error from attempt 2 onward', async () => {
        ...
        await runWithRecovery(runner, { availableSigners: ['did:web', 'did:key'] }, { onAttempt });
    ...
    it('refuses empty availableSigners', async () => {
        await expect(runWithRecovery(async () => 'x', { availableSigners: [] })).rejects.toThrow(
            /availableSigners must be non-empty/
        );
    });
