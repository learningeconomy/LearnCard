...
import type { QrLoginSession, QrPayload, QrLoginSessionInfo } from '@learncard/sss-key-manager';

// ---------------------------------------------------------------------------
// Requester Hook (Device B — the new device)
// ---------------------------------------------------------------------------

export type QrRequesterStatus = 'idle' | 'creating' | 'waiting' | 'approved' | 'expired' | 'error';
...
export const useQrLoginRequester = (
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            if (
                message.includes('Web Crypto') ||
                message.includes('crypto.subtle') ||
                message.includes('secure context')
            ) {
                message =
                    'Secure connection required. Cross-device sign-in is not available over HTTP. Please use a production build or HTTPS.';
            }
...
export type QrApproverStatus = 'idle' | 'loading' | 'confirming' | 'approving' | 'done' | 'error';
