...
import { useWallet, SelectedCredsStoreState, getBaseUrl } from 'learn-card-base';
    ...
    const handleSubmit = async (payload: SelectedCredsStoreState) => {
        ...
        try {
            ...
            const selectedCredentials = allCredentials?.filter((cred: any) => {
                ...
                const duplicated = duplicate.has(cred.id);
                duplicate.add(cred.id);
                return !duplicated && selected;
            });
    ...
    return (
        <IonPage className="modal-wrapper">
            ...
            <IonContent className="course-details-container bg-white h-full" fullscreen>
                {page === 'application' && !loading && <ShareCredentials onSubmit={handleSubmit} />}
