    ...
    return (
        <div className="flex items-start gap-3 p-4 rounded-2xl bg-grayscale-10 border border-grayscale-200">
            ...
            <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-grayscale-500 mb-0.5">{caption}</p>

                <p className="text-base font-semibold text-grayscale-900 truncate">{displayName}</p>
 ...
 * fallback.
 */
const VerifierAvatar: React.FC<VerifierAvatarProps> = ({ logoUri, domain, seed, altText }) => {
...
const avatarGradient = (seed: string): { background: string; initials: string } => {
    if (!seed) {
        return {
    ...
    const cleaned = seed
        ...
        .split(/[/.:]/)
        .filter(Boolean);
    const sld = cleaned.length >= 2 ? cleaned[cleaned.length - 2] : cleaned[0] ?? seed;
