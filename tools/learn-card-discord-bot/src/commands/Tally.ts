import { BaseCommandInteraction, Client, Message, MessageReaction } from 'discord.js';
...
const constructLineItem = (wrapper: LmaoReactionWrapper, ix: number): string =>
    `${getRankIcon(ix + 1)} <@!${wrapper.m.author.id}> \u2014 **"${wrapper.m?.content}"** \u2014 ${
        wrapper.r?.count || 0
...
export const constructTallyMessage = async (client: Client, channelId: string): Promise<string> => {
    ...
    if (test && test.type === 'GUILD_TEXT') {
        ...
        const messages = await test.messages.fetch({}, { cache: false, force: true });
