...
| `doctor`                                                                                     | Read-only preflight: identity, network, signing service, token scopes, signing authority (test-sign + verify), `did:web` document, webhook (`--webhook-url`), credential refresh, Trusted Registry. `--strict` fails on warnings.                                                 |
...
| `serviceAccounts[]`            | `{ name, scopes[], expiresAt? }`        | One auth grant + API token each. Token written once to `--secrets-out` as `NAME=…` (upper-cased, `-`→`_`). Existing grants are never re-fetched. |
...
| `secrets.env` (or `--secrets-out`) | Service-account tokens. Mode 0600, gitignored. The CLI itself never reads it — it is for your backend.                                                                                           |
