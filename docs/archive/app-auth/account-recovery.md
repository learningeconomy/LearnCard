...
This section explains how LearnCard users recover access to their private key when they lose their device, clear their browser storage, or sign in on a new device. Recovery is built on the [SSS key management](../../core-concepts/identities-and-keys/key-management-sss.md) system — specifically, the ability to reconstruct the private key from any two of its four shares.
