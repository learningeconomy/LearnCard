...
- Private keys are **never stored whole** — at rest they exist only as shares. During a session the key is reconstructed in memory on the user's device for signing; it is not persisted.
