...
A DID is a username that no company owns. It's a globally unique identifier (like `did:key:z6Mk...`) that a person or organization controls themselves — so their identity, and the credentials attached to it, keep working even if any single platform disappears.
