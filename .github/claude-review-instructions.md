...
Approvals and review comments are submitted by running the `gh` CLI
in the Bash tool — `gh` is pre-authenticated as a GitHub App that is
permitted to approve PRs. Always cast the decision with the exact
command shown; do not rely on any built-in review tool.

**Approve** — PR matches a lane, passes all hard-stop checks, and is within LOC limits:
`gh pr review $PR_NUMBER --approve --body "Auto-approved (Lane N — <lane name>): <one-line reason>"`

**Comment-only / manual review required** — PR is outside auto-approval scope but nothing is wrong:
`gh pr review $PR_NUMBER --comment --body "<concise summary of review findings and why manual review is needed>"`

**Flag concern** — PR contains something that looks risky, suspicious, or incorrect:
`gh pr review $PR_NUMBER --comment --body "⚠️ <description of concern>\n\n<rest of review>"`

### Important Rules

- NEVER auto-approve your own changes (commits authored by `claude` or `github-actions`).
- NEVER auto-approve if any hard-stop rule is triggered, even if a label suggests otherwise.
- A label like `auto-approve/docs` is a hint, not an override. Always verify the files actually match the lane.
- When in doubt, choose "comment-only." It is always safe to leave a thorough review without approving.
