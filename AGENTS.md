...
-   A footer shared between a border-owning modal root and a non-modal page may compose with `--ion-safe-area-bottom`: the modal ownership boundary resets that variable to zero while the page inherits the device inset. `AdminToolsModalFooter` uses this pattern.
