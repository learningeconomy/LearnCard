...
LC-1811's own success criteria included a stop-condition: _"If we measure the current path and the cost is something trivially fixable without re-architecture, ship the fix and stop."_ We applied that test.
...
The LC-1644 ticket's original success criterion was _"end-to-end claim completes in under 4 seconds on a typical network."_ We're under that target by a comfortable margin even from Japan.
...
> _"How do we eliminate the 5–10s latency that every signing-authority-backed issuance pays today — without putting partner seeds inside brain-service, and without breaking the partner-hosted Signing Authority contract?"_
