...
const DEFAULT_OPENSALT_STAGING_FRAMEWORK_REF = 'https://staging.opensalt.net/cftree/doc/2695';
...
describe('Skills OpenSALT Staging E2E', () => {
    test('shows public OpenSALT Staging frameworks but not other users private frameworks', async ({
        skip,
    }) => {
        if (process.env.E2E_REAL_OPENSALT_STAGING !== 'true') {
            skip();
            return;
        }
        ...
        const frameworkRef =
            process.env.E2E_OPENSALT_STAGING_FRAMEWORK_ID || DEFAULT_OPENSALT_STAGING_FRAMEWORK_REF;
    ...
    test('links, syncs, aligns, issues, and verifies using real OpenSALT Staging skills', async ({
        skip,
    }) => {
        if (process.env.E2E_REAL_OPENSALT_STAGING !== 'true') {
            skip();
        ...
        if (!skillToAlign?.id) throw new Error('No OpenSALT Staging skill available for alignment');
