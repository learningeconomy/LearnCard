...
const findSdJwtConfigId = async (): Promise<{
    ...
    const res = await fetch(`${ISSUER_BASE_URL}/draft13/.well-known/openid-credential-issuer`);
    if (!res.ok) {
        throw new Error(`Could not fetch walt.id issuer metadata: HTTP ${res.status}`);
    }
...
describe('interop: SD-JWT VC issuance against walt.id', () => {
    it('accepts a walt.id SD-JWT offer and receives the credential', async () => {
        ...
        const rawOfferUri = await mintWaltidOffer({
            ...
            credentialType:
                typeof (config as any).vct === 'string'
