...
- **D12 (Software Mode Blocked):** Software mode is blocked in production and staging builds. Staging escrow testing must use the Nitro backend to ensure realistic testing.
...
- **D14 (Release Replay Refused):** Release replay is explicitly refused after a `Released` state is observed. A crash or lost response after append but before returning ciphertext burns the hold.
...
| Release replay                  | **Prevented**. Once `Released` is observed, every retry is refused. A crash after append but before returning ciphertext burns the hold.                                                                                                                                     | `services/escrow-enclave-app/README.md`, [Design Decisions](#design-decisions) (D14)     |
...
| Software mode in staging        | **Prevented**. Software mode is blocked in production and staging builds. Staging must use real Nitro enclaves.                                                                                                                                                              | [Design Decisions](#design-decisions) (D12)                                              |
...
- **BLOCKER-AUDIT:** The `cargo audit` CI gate fails due to transitive vulnerabilities in the legacy AWS SDK TLS stack (`h2`, `rustls-webpki`). These must be resolved or explicitly ignored with documented rationale before the CI pipeline can pass.
