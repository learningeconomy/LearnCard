...
export const getDeliveryService = (contactMethod: ContactMethod): DeliveryService => {
    ...
    if (contactMethod.type === 'phone') {
        const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN } = environment;
        ...
        const { MESSAGEBIRD_AUTH_TOKEN, MESSAGEBIRD_ORIGINATOR } = environment;
    ...
    if (contactMethod.type === 'email') {
        if (environment.POSTMARK_API_KEY) {
            console.log('Postmark API Key found. Using PostmarkAdapter.');
            return new PostmarkAdapter(environment.POSTMARK_API_KEY);
        }
