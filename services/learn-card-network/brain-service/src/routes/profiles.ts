  ...
  getProfile: openProfileRoute
    .meta({
      openapi: {
        ...
        description: 'This route uses the request header to grab the profile of the current user',
      },
    })
    .input(z.void())
  ...
  getOtherProfile: openRoute
    .meta({
      openapi: {
        ...
        description: 'This route grabs the profile information of any user, using their profileId',
      },
    })
        ...
        const profilesWithConnectionStatus = await Promise.all(
          profiles.map(async profile => {
            ...
            if (targetProfileFull) {
              return {
                ...profile,
                connectionStatus: await getConnectionStatus(selfProfile, targetProfileFull),
              };
            }
            return profile;
          })
