...
const handleSendCredentialEvent = async (
    ...
    // LC-1644 bench: the optional `perfTracker` arg is accepted for API
    // compatibility with the optimized branch (feat/lc-1644-appevent-perf), but
    // unused here — this branch lacks the per-phase instrumentation Tasks 4/5
    // add to handleSendCredentialEvent. The bench loop's own tracker still
    // captures total_ms around the call, which is the comparison-meaningful
    // signal for "before vs after optimizations".
    void perfTracker;
