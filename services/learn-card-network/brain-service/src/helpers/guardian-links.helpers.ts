...
export async function claimPendingGuardianLinksForProfile(
    ...
    console.log(
        '[claimPendingGuardianLinks] Starting for profile:',
        profile.profileId,
        'did:',
        profile.did
    );
    ...
    const verifiedEmail = contactMethods.find(cm => cm.type === 'email' && cm.isVerified);
    if (!verifiedEmail) {
        console.log(
            '[claimPendingGuardianLinks] No verified email found. ContactMethods:',
    ...
    console.log(
        '[claimPendingGuardianLinks] Approved credentials found:',
    ...
    for (const inboxCredential of approvedCredentials) {
        try {
            ...
            if (!childProfile) {
                console.log(
                    '[claimPendingGuardianLinks] No child profile found for credential:',
                    inboxCredential.id
                );
                continue;
            }
            console.log(
                '[claimPendingGuardianLinks] Found child profile:',
            ...
            if (!alreadyManages) {
                console.log(
                    '[claimPendingGuardianLinks] Creating MANAGES relationship:',
                    profile.profileId,
                    '->',
                    childProfile.profileId
                );
                ...
                console.log(
                    '[claimPendingGuardianLinks] MANAGES relationship created, managerId:',
                    managerId
                );
            } else {
                console.log(
                    '[claimPendingGuardianLinks] Already manages child:',
                    childProfile.profileId
                );
            }
            ...
            console.error(
                '[claimPendingGuardianLinksForProfile] Failed to process credential:',
                inboxCredential.id,
                err
            );
        }
