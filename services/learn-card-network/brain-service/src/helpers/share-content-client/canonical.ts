...
/**
 * C1-canonical request-body serializer, implemented locally so brain-service
 * never imports LearnCloud runtime code.
 *
 * This mirrors `services/learn-card-network/learn-cloud-service/src/helpers/
 * share-content-auth/canonical.ts` exactly:
 * - object keys sorted recursively;
 * - strings JSON-encoded (unpaired surrogates rejected);
 * - only safe integers, never negative zero;
 * - sparse arrays, cycles, non-plain objects and unsupported value types
 *   rejected instead of coerced;
 * - `undefined` values are rejected, not dropped.
 *
 * The cross-implementation contract test pins this against fixed C1 vectors.
 */
