...
describe('Contact Methods', () => {
    ...
    describe('Get my contact methods', () => {
        ...
        describe('Verify With Credential (proof-of-login VP-JWT)', () => {
            ...
            it('should reject a VP-JWT whose holder is not a trusted login provider', async () => {
                ...
                await expect(
                    userB.clients.fullAuth.contactMethods.verifyWithCredential({
                        proofOfLoginJwt: vpJwt,
                    })
        ...
        it('should not let a user get their contact methods without full auth', async () => {
            await expect(noAuthClient.contactMethods.getMyContactMethods()).rejects.toMatchObject({
                code: 'UNAUTHORIZED',
            });
            await expect(
        ...
        it('should allow adding a contact method', async () => {
            const contactMethod = await userB.clients.fullAuth.contactMethods.addContactMethod({
                type: 'email',
            ...
            expect(new Date(contactMethods?.[0].createdAt).getTime()).toBeLessThanOrEqual(
                new Date().getTime()
            );
        });
        ...
        it('should allow verifying an email contact method', async () => {
            ...
            expect(new Date(verifiedMethod?.verifiedAt).getTime()).toBeLessThanOrEqual(
                new Date().getTime()
            );
        });
        ...
        it('should not allow verifying a phone contact if the profiles DID is not in the trusted issuers whitelist', async () => {
            await expect(
                userB.clients.fullAuth.contactMethods.addContactMethod({
                    type: 'phone',
                    value: '+15555555',
                })
            ).rejects.toThrow();
        ...
        it('should not allow verifying a contact method that does not exist', async () => {
            await expect(
                userB.clients.fullAuth.contactMethods.verifyContactMethod({
                    token: 'invalid-token',
                })
        ...
        it('should allow setting a contact method as primary', async () => {
            ...
            await userB.clients.fullAuth.contactMethods.setPrimaryContactMethod({
                contactMethodId,
            });
        ...
        it('should unset previous primary contacts when setting a new primary contact', async () => {
            ...
            const { contactMethodId: contactMethodId2 } =
                await userB.clients.fullAuth.contactMethods.addContactMethod({
                    type: 'phone',
                    value: '+15555555',
                });
        ...
        it('should allow removing a contact method', async () => {
            ...
            await userB.clients.fullAuth.contactMethods.removeContactMethod({
                id: contactMethodId,
            });
    ...
    describe('Create Inbox Claim Session', () => {
        beforeEach(async () => {
            ...
            integration = await userA.clients.fullAuth.integrations.getIntegration({
                id: integrationId,
            });
        });
        ...
        it('should return UNAUTHORIZED when creating a contact method session with an incorrect otp code', async () => {
            ...
            // Add a contact method to user A
            await userA.clients.fullAuth.contactMethods.sendChallenge({
                type: 'email',
                value: 'userA@test.com',
            ...
            await expect(
                userB.clients.fullAuth.contactMethods.createContactMethodSession({
