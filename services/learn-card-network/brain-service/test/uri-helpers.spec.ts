...
vi.mock('@accesslayer/consentflowcontract/relationships/read', () => ({
    getContractTermsByUri: vi.fn(),
}));
...
describe('escapeColonsInDomain', () => {
    ...
    it('encodes localhost colon when no /trpc: present', () => {
        ...
        expect(escapeColonsInDomain(uri)).toBe('lc:network:localhost%3A4000:boost:abc123');
    });
...
describe('getIdFromUri', () => {
    ...
    it('extracts ID from preview URI', () => {
        expect(getIdFromUri('lc:network:pr-99.preview.learncard.ai/brain/trpc:boost:abc123')).toBe(
            'abc123'
        );
    });
...
describe('getDomainFromUri', () => {
    ...
    it('strips /trpc suffix from preview domain', () => {
        expect(
            getDomainFromUri('lc:network:pr-99.preview.learncard.ai/brain/trpc:boost:abc123')
        ).toBe('pr-99.preview.learncard.ai/brain');
    });
