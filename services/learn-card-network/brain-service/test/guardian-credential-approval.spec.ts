...
describe('Guardian-Gated Credential Issuance', () => {
    ...
    describe('send with guardianEmail', () => {
        ...
        it('should send exactly 2 emails: one to student, one to guardian', async () => {
            ...
            const studentEmail = calls.find(
                (c: any) => c.contactMethod.value === 'student@school.edu'
            );
            const guardianEmail = calls.find(
                (c: any) => c.contactMethod.value === 'parent@home.com'
            );
            ...
            expect(guardianEmail?.templateModel?.approvalUrl).toContain(
    ...
    describe('sendGuardianChallenge', () => {
        it('should send OTP email to guardian and return success message', async () => {
            ...
            const token = await generateGuardianCredentialApprovalToken(
                inboxCred.id,
                'parent@home.com'
            );
    ...
    describe('approveGuardianCredential', () => {
        ...
        beforeEach(async () => {
            ...
            approvalToken = await generateGuardianCredentialApprovalToken(
                inboxCredId,
                'parent@home.com'
            );
        });
        ...
        it('should approve and send student notification when token + OTP are valid', async () => {
            ...
            const otpCall = sendSpy.mock.calls.find(
                (c: any) => c[0].templateId === 'guardian-email-otp'
            );
        ...
        it('should not approve again with the same OTP (code is consumed)', async () => {
            ...
            await userA.clients.fullAuth.inbox.approveGuardianCredential({
                token: approvalToken,
                otpCode,
            });
    ...
    describe('approveGuardianCredentialInApp', () => {
        ...
        beforeEach(async () => {
            ...
            const { getContactMethodByValue } = await import(
                '../src/accesslayer/contact-method/read'
            );
        ...
        it('should approve when caller has MANAGES relationship with the child', async () => {
            ...
            const { createProfileManager } = await import(
                '../src/accesslayer/profile-manager/create'
            );
    ...
    describe('claimPendingGuardianLinks', () => {
        ...
        it('should establish MANAGES relationship and return child info for approved credentials', async () => {
            ...
            const credentials = await InboxCredential.findMany({ where: {} });
            const inboxCred = credentials[0]!;
            await updateInboxCredential(inboxCred.id, {
            ...
            const { createContactMethod } = await import(
                '../src/accesslayer/contact-method/create'
            );
        ...
        it('should be idempotent — second call returns managerId: null for already-linked children', async () => {
            ...
            const credentials = await InboxCredential.findMany({ where: {} });
            await updateInboxCredential(credentials[0]!.id, {
                guardianStatus: 'GUARDIAN_APPROVED',
                isAccepted: true,
            });
