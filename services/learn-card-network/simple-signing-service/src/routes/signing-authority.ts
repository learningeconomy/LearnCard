...
export const signingAuthorityRouter = t.router({
    createSigningAuthority: didAndChallengeRoute
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            // Use provided ownerDid or fall back to authenticated user's DID
            const effectiveOwnerDid = ownerDid ?? ctx.user.did;

            if(await getSigningAuthorityForDid(effectiveOwnerDid, name)) {
                throw new TRPCError({
                    code: 'CONFLICT',
                    message: `Signing Authority with name, ${name}, already exists for user.`,
                });
            }
            const createdId = await createSigningAuthorityForDID(effectiveOwnerDid, name);
            if(!createdId) {
                throw new TRPCError({
